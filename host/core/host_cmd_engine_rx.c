...
s32 leave_host_event_handler_(struct cfg_host_event *pPktInfo);
s32 join_host_event_handler_(struct cfg_host_event *pPktInfo);
s32 scan_host_event_handler_(struct cfg_host_event *pPktInfo);
s32 scan_done_host_event_handler_(struct cfg_host_event *pPktInfo);
...
extern bool mlme_remove_ap(struct ssv6xxx_ieee80211_bss *bss);
extern void ieee80211_delete_all_ampdu_rx(void);
...
s32 leave_host_event_handler_(struct cfg_host_event *pPktInfo)
{
    ...
    if((vif)&&(vif->hw_mode == SSV6XXX_HWM_STA))
    {
        ...
        if (gDeviceInfo->ampduRx_vif == leave_res->u.leave.bssid_idx)
        {
        	/* g_host_cfg.ampdu_rx_enable= AMPDU_RX_ENABLE(0) */
...
#endif
    bool pass_to_up_layer=TRUE;
    join_res = (struct resp_evt_result *)pPktInfo->dat;
    LOG_PRINTF("join_host_event_handler,vif_idx=%d,status_code=%d\r\n",join_res->u.join.bssid_idx,join_res->u.join.status_code);
...
u16 CmdEng_GetRawRxDataOffset(CFG_HOST_RXPKT *p)
{
    u16 offset=0;
...
u8 *ssv6xxx_host_rx_data_get_data_ptr(CFG_HOST_RXPKT *rxpkt)
{
