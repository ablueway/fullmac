...
#endif
static int _ssv6030_hal_beacon_fill_content(u32 regaddr, u8 *beacon, int size);
static int _ssv6030_hal_beacon_set_info(u8 beacon_interval, u8 dtim_cnt);
static int _ssv6030_hal_beacon_set_id_dtim(u8 availableBcn, u32 pbufAddr, u32 dtim_offset);
static int _ssv6030_hal_beacon_reg_lock(bool block);
static int _ssv6030_hal_beacon_fill_tx_desc(u8 bcn_len, void *frame);
...
int ssv6030_hal_beacon_enable(bool bEnable)
{
    ...
    {
        MEMSET(hw_bcn_info,0,sizeof(struct ssv6xxx_beacon_info)*bcn_cnt);
        beacon_usage =0;
        beacon_enable = FALSE;
        if(gDeviceInfo->recovering != TRUE)
...
{
    u32 val;
    u32 reg_tx_beacon_adr[] = {ADR_MTX_BCN_CFG0, ADR_MTX_BCN_CFG1};
    val = (SSV6051_PBUF_MapPkttoID(pbufAddr))|(dtim_offset<<MTX_DTIM_OFST0);
    MAC_REG_WRITE( reg_tx_beacon_adr[availableBcn], (u32)val);
    return 0;
}
