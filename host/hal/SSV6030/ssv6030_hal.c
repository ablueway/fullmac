...
static bool _ssv6030_do_firmware_checksum(u32 origin);
#if(DO_IQ_CALIBRATION==1)
static int _ssv6030_hal_do_iq_calib(void);
#endif
static bool _ssv6030_hal_mcu_input_full(void);
static bool _ssv6030_set_hw_table(const ssv_cabrio_reg tbl_[], u32 size);
...
bool cabrio_resource_setup(u8 param)
{
    bool ret = FALSE;
    u32 id_len = 0;
    u8 rx_id_threshold = SSV6051_ID_RX_THRESHOLD;
    u8 rx_page_threshold = SSV6051_PAGE_RX_THRESHOLD;
...
int ssv6030_hal_set_ext_rx_int(u32 pin);
int ssv6030_hal_set_wakeup_bb_gpio(bool en);
int ssv6030_hal_init_mac(u8 *self_mac)
{
    int i;
    bool ret;
    u32 regval,j;
...
#endif
    if(ret == TRUE) ret = MAC_REG_WRITE(ADR_STA_MAC_0, *((u32 *)&(self_mac[0])));
    if(ret == TRUE) ret = MAC_REG_WRITE(ADR_STA_MAC_1, *((u32 *)&(self_mac[4])));
    /**
...
//PHY and security table
    ...
    hw_buf_ptr = ssv_hal_pbuf_alloc((s32)sizeof(phy_info_tbl)+
   ...
   /*lint -restore */
    ...
    if(gDeviceInfo->recovering != TRUE)
        ...
        __FUNCTION__, hw_buf_ptr, (u32)(sizeof(phy_info_tbl)+sizeof(struct ssv6xxx_hw_sec)));
...
*/
    ...
    for(i=0, ptr=(u32 *)phy_info_tbl; i<PHY_INFO_TBL1_SIZE; i++, ptr++) {
        regval = *ptr;
    ...
    if(!g_host_cfg.extRxInt)
        ssv6030_hal_set_wakeup_bb_gpio(0);
    else
...
int ssv6030_hal_init_sta_mac(u32 wifi_mode)
{
    int i, ret=0;
    u32 regval;
    u16 *mac_deci_tbl;
...
int ssv6030_hal_setup_ampdu_wmm(bool IsAMPDU)
{
    if(IsAMPDU == TRUE)
    {
#if (AUTO_BEACON != 1)
...
int ssv6030_hal_init_ap_mac(u8 *bssid,u8 channel)
{
    int i, ret=0;
    u32 temp;
    u16 *mac_deci_tbl;
...
int ssv6030_hal_ap_wep_setting(ssv6xxx_sec_type sec_type, u8 *password, u8 vif_idx, u8* sta_mac_addr)
{
    ...
    switch (sec_type)  {
        case SSV6XXX_SEC_WEP_40:
            ...
            MEMCPY(sec_entry.key,  password, strlen((void*)password));
...
int ssv6030_hal_sw_reset(u32 comp)
{
    switch (comp)
    {
        case SW_RESET_MAC:
...
int ssv6030_hal_pause_resuem_recovery_int(bool resume)
{
    u32 val;
...
int ssv6030_hal_set_TXQ_SRC_limit(u32 qidx,u32 val)
{
    u32 tmp;
    if(val < 1 || val > 7)
        return;
...
int ssv6030_hal_halt_txq(u32 qidx,bool bHalt)
{
    u32 regval;
...
static inline void _SET_RG_SARADC_THERMAL(u32 _VAL_)  
{
    u32 value = 0;
...
static inline void _SET_RG_EN_SARADC(u32 _VAL_)
{ 
    u32 value = 0; 
    MAC_REG_READ(0xce010030, value); 
...
static inline u32 _GET_SAR_ADC_FSM_RDY() 
{
    u32 value = 0; 
...
static inline u32 _GET_RG_SARADC_BIT() 
{
    u32 value = 0;
    MAC_REG_READ(0xce010094, value);
...
int ssv6030_hal_get_temperature(u8 *sar_code, s8 *temperature)
{
    u32 _sar_code=0;
...
int ssv6030_hal_set_volt_mode(u32 mode)
{
...
int ssv6030_hal_gen_random(u8 *data, u32 len)
{
    u32 idx=0;
    u32 rtemp;
...
int ssv6030_hal_get_rssi_from_reg(u8 vif_idx)
{
    u32 regVal=0;
...
int ssv6030_hal_set_agc_gain(u32 gain)
{
...
int ssv6030_hal_get_rc_info(u16 *tx_cnt, u16 *retry_cnt, u16 *phy_rate)
{
    u32 regval = 0;
...
bool ssv6030_hal_get_diagnosis(void)
{
    u32 SysIrq=0;
    u32 Wdg=0;
    u32 ChipId0;
...
int ssv6030_hal_get_fw_status(u32 *val)
{
    MAC_REG_READ(SSV6051_FW_STATUS_REG,(*val));
    return 0;
}

int ssv6030_hal_set_fw_status(u32 val)
{
...
int ssv6030_hal_set_short_slot_time(bool enable)
{
    u32 regval=0;
...
int ssv6030_hal_get_tx_resources(u16 *pFreePages, u16 *pFreeIDs, u16 *pFreeSpaces)
{
...
int ssv6030_hal_bytes_to_pages(u32 size)
{
    u32 page_count=0;
...
int ssv6030_hal_download_fw(u8 *fw_bin, u32 fw_bin_len)
{
    bool ret = FALSE;
...
#define IQK_ALIGN_SIZE(size) (((size) + 4 - 1) & ~(4-1))
    ...
    hwPBUF=(u32)ssv_hal_pbuf_alloc(len,TX_BUF);
    if((s32)hwPBUF==-1)
    {
    ...
    *((u8 *)((u32)host_cmd->un.dat8+IQK_CFG_LEN))=((u32)hwPBUF)&0xFF;
    *((u8 *)((u32)host_cmd->un.dat8+IQK_CFG_LEN+1))=((u32)hwPBUF>>8)&0xFF;
    *((u8 *)((u32)host_cmd->un.dat8+IQK_CFG_LEN+2))=((u32)hwPBUF>>16)&0xFF;    
    *((u8 *)((u32)host_cmd->un.dat8+IQK_CFG_LEN+3))=((u32)hwPBUF>>24)&0xFF;        
...
int ssv6030_hal_tx_loopback_done(u8 *dat)
{
    MEMCPY(&tx_loopback_info,dat,sizeof(struct cfg_tx_loopback_info));
...
int ssv6030_hal_update_cci_setting(u16 input_level)
{
    return -1;
}
#else
...
bool ssv6030_hal_support_5g_band(void)
{
    return FALSE;
}
const struct ssv_hal_ops	g_hal_ssv6030=
{
    ...
    ssv6030_hal_set_wakeup_bb_gpio,
