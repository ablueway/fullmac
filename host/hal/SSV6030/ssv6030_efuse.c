...
#endif
static u8 _ssv6030_hal_read_efuse(u8 *pbuf)
{
    u32 *pointer32, i;
    pointer32 = (u32 *)pbuf;
...
static u16 _ssv6030_hal_parser_efuse(u8 *pbuf, u8 *mac_addr)
{
    u8 *rtemp8,idx=0;
    u16 shift=0,i;
    ...
    do
    {
        ...
        switch(idx)
        {
            ...
            if(shift)
            {
                ...
                SSV_EFUSE_ITEM_TABLE[idx].value = (u16)((u8)(value) & ((1<< SSV_EFUSE_ITEM_TABLE[idx].byte_cnts) - 1));
            }
            else
            {
                OS_MemCPY((void*)&value,(void*)rtemp8,2);
                SSV_EFUSE_ITEM_TABLE[idx].value = (u16)((u8)(value >> 4) & ((1<< SSV_EFUSE_ITEM_TABLE[idx].byte_cnts) - 1));
            }
...
int ssv6030_hal_read_chip_id(void)
{
    u32 val=0;
    u8  cnt=10;
