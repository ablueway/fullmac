                                       ...
                                       bool f80211, u8 security, u8 tx_dscrp_flag,u8 vif_idx)
{
    ...
    if (f80211 == 0) {
        ...
        if (NULL!=addr4) {
            MEMCPY(pos, addr4, ETHER_ADDR_LEN);
            pos += ETHER_ADDR_LEN;
            baddr4=TRUE;
        }
...
u8 *ssv6030_hal_get_rxpkt_data_ptr(void *_rxpkt)
{
    ...
    {
        return (u8*)(((u8*)rxpkt)+SSV6051_RXINFO_SIZE);
    }
}
...
int ssv6030_hal_get_rxpkt_data_len(void *rxpkt)
{
    ...
    if(_p->f80211==0)
    {
        u8 *raw = ssv6030_hal_get_rxpkt_data_ptr(_p);
        return _p->len - (u32)(raw-(u8*)_p);
    }
    else
    {
...
u8 ssv6030_hal_get_rxpkt_bssid_idx(void *rxpkt)
{
