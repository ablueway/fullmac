...
#if(SW_8023TO80211==1)
u32 _ssv6006_hal_tx_8023to80211(void *frame, u8 l3offset, u16 *qos, u8 security,u8 up)
{
    ETHER_ADDR da, sa;
    u8 ds;
    u8 *raw=NULL;
    bool bqos=FALSE,bht=FALSE,baddr4=FALSE;
    u16 eth_type;
    u8 fromDS, toDS;
    u32 _ht=0;
    u16 _qos=0;
    ...
    if(qos!=NULL)
    {
        ...
        _qos = ((*qos)&0xFFF8)|up;
        OS_MemCPY(raw,(u8 *)(&_qos),IEEE80211_QOS_CTL_LEN);
        bqos=TRUE;
    }
    ...
    return ((u32)raw-(u32)OS_FRAME_GET_DATA(frame));
}

u8 _wsid_lookup(ETHER_ADDR *da)
{
    u8 index, max_index=WLAN_MAX_STA;
...
extern u32 g_ap_tx_data_flow;
extern u32 g_ap_tx_mgmt_flow;
extern u32 g_sta_tx_data_flow;

static int _ssv6006_fill_txinfo(void *frame)
{
    u8 AC_TBL[8]={ AC_BE, AC_BK, AC_BK, AC_BE, AC_VI, AC_VI, AC_VO, AC_VO };
    PKT_TxInfo *PktInfo=NULL;
    u8 *raw=NULL;
    u8 *p=NULL;
    ETHER_ADDR *da=NULL;
    u16 fc=0;
    u8 *qos, tid, bad_len, hdr_len=24, ack_policy=0,wsid;
    u32 i=0;
    u32 temp=0;
    u8 hdr_offset=SSV6006_TX_PB_OFFSET;
    ...
    raw=(u8 *)((u32)PktInfo+sizeof(PKT_TxInfo)+padding_len);
    ...
    */
    ...
    else
    {
        ...
        p=(u8 *)((u32)PktInfo+4);
        p[0]=((u8 *)(&g_sta_tx_data_flow))[0];
        p[1]=((u8 *)(&g_sta_tx_data_flow))[1];
        p[2]=((u8 *)(&g_sta_tx_data_flow))[2];
        p[3]=((u8 *)(&g_sta_tx_data_flow))[3];
        //LOG_PRINTF("WORD_2=0x%08x\r\n",g_sta_tx_data_flow);
    }
                                       ...
                                       bool f80211, u8 security)
{
    ...
    */
    ...
    _ssv6006_hal_tx_8023to80211(frame,(u32)push_len,qos,security,priority);
                                       ...
                                       bool f80211, u8 security, u8 tx_dscrp_flag, u8 vif_idx)
{
    ...
    OS_MemSET(host_txreq0,0,sizeof(SSV6006_HOST_TXREQ0));
    pos = (u8*)host_txreq0 + ssv_hal_get_txreq0_size();
...
#define IS_4_BYTE_ALIGN(x)              ((((u32)(x))&0x03) == 0)
#define IS_2_BYTE_ALIGN(x)              ((((u32)(x))&0x01) == 0)
...
int ssv6006_hal_rx_80211to8023(void *_PktRxInfo)
{
    ...
    u16 fc;
    u8 rfc1042[]={ 0xAA, 0xAA, 0x03, 0x00, 0x00, 0x00 };
    ...
    ETHER_ADDR a4;
	u32 ht_ctl;
    u16 encap_offset=0,qos_ctl;
    dat = (u8 *)((u32)PktRxInfo + PktRxInfo->hdr_offset);
    hdr_data = (HDR80211_Data *)dat;
    fc = dat[0] | (dat[1]<<8);
    //dat = (u8 *)PktRxInfo + PktRxInfo->payload_offset;
    dat = (u8 *)PktRxInfo + GET_PKT_PAYLOAD_OFFSET(PktRxInfo);
    ...
    if (IS_DATA_FRAME(fc))
    {
       ...
       */
       ...
       //ssv6xxx_raw_dump((char *)_PktRxInfo,128);
        ...
        //return 0;
        /* DMAC & SMAC: */
        dat = (u8 *)PktRxInfo + PktRxInfo->hdr_offset;
...
u8 *ssv6006_hal_get_rxpkt_data_ptr(void *_rxpkt)
{
    ...
    if(rxpkt->f80211==0)
    {
        ...
        return (u8*)(((u8*)rxpkt)+offset);
    }
    else
    {
        //LOG_PRINTF("\33[35m %s():%d \33[0m\r\n",__func__,__LINE__);
        return (u8*)(((u8*)rxpkt)+SSV6006_RXINFO_SIZE);
    }
}
...
int ssv6006_hal_get_rxpkt_data_len(void *rxpkt)
{
    ...
    if(_p->f80211==0)
    {
        ...
        #else
        u8 *raw = ssv6006_hal_get_rxpkt_data_ptr(_p);
        return _p->len - (u32)((u32)raw-(u32)_p);
        #endif
    }
    else
    {
...
u32 ssv6006_hal_process_hci_rx_aggr(void* pdata, u32 data_length, RxPktHdr cbk_fh)
{
    ...
    void * rFrame = NULL;
    u8  *msg_data = NULL, *aggrdata=(u8*)pdata;
    SSV6006_HOST_RXPKT *_p=NULL;
    u32 next_aggr_len=0,next_aggr_len2=0;
    u8 rxaggrcnt=0;
...
{
    u32 align_size = (((OS_FRAME_GET_DATA_LEN(tFrame)) + 4 - 1) & ~(4 - 1));
    u8* start_addr = (u8*)aggr_buf;
    ...
    *aggr_len=0;
    ...
    start_addr += (align_size+sizeof(struct hci_tx_aggr_info));
    aggr_tx_info = (struct hci_tx_aggr_info*)(start_addr);    
