...
#if (CLI_HISTORY_ENABLE == 1)
void cli_init_history_parms(void)
{
    u8 idx = 0;
    gCmdHistoryIdx = 0;
    gCmdHistoryCnt = 0;
	/* CLI_HISTORY_NUM(10) */
    for (idx = 0; idx < CLI_HISTORY_NUM; idx++)
	{    	
		gCmdHistoryBuffer[idx][0] = 0x00;      
	}
}
#endif

void cli_init_global_parms(void)
{
    gCmdBuffer[0] 	= 0x00;
    sgCurPos 		= 0;
    sgArgC 			= 0;
...
{
    u8 idx = 0;
	if (OS_MsgQCreate(&g_cli_task_info[0].qevt, 
		(s32)g_cli_task_info[0].qlength) != OS_SUCCESS)
	{
	    LOG_PRINTF("OS_MsgQCreate faild\r\n");
	    return;
	}
	for (idx = 0; idx < sizeof(g_cli_task_info) / sizeof(struct task_info_st); idx++) 
	{
	    OS_TaskCreate(g_cli_task_info[idx].task_func,
					    g_cli_task_info[idx].task_name,
					    g_cli_task_info[idx].stack_size<<4,
					    NULL,
					    g_cli_task_info[idx].prio,
					    NULL);
	}
}
