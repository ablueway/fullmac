...
void _cmd_wifi_sconfig (s32 argc, char *argv[])
{
     ...
     */
    u16 channel_mask = (u16)strtol(argv[1],NULL,16);
    u32 channel_5g_mask = (u32)strtol(argv[2],NULL,16);  
...
void _cmd_wifi_scan (s32 argc, char *argv[])
{
     ...
     */
    u16 channel_mask = (u16)strtol(argv[0],NULL,16);
    u32 channel_5g_mask = (u32)strtol(argv[1],NULL,16);
...
void _cmd_wifi_join (s32 argc, char *argv[])
{
    ...
    if (argc > arg_cnt)
    {
        STRCPY((char *)join_cfg->ssid.ssid, argv[arg_cnt]);
        join_cfg->ssid.ssid_len=STRLEN(argv[arg_cnt]);
        if(argc > (arg_cnt+1))
        {
            arg_cnt++;
            STRCPY((char *)join_cfg->password, argv[arg_cnt]);
        }
    }
    else
    {
...
void _cmd_wifi_leave(s32 argc, char *argv[])
{
    ...
    if(argc > 0)
        vif_idx = (u8)ssv6xxx_atoi(argv[0]);
    ...
    for (i=0; i<AP_cnt; i++)
    {
        ...
        if(ap_list[i].channel_id!= 0)
		{
            ...
            if((connected_info.vif_operate[0]==SSV6XXX_HWM_STA)&&
              (!memcmp((void *)ap_list[i].bssid.addr,(void *)connected_info.vif_u[0].station_status.apinfo.Mac,ETHER_ADDR_LEN)))
            {
                    LOG_PRINTF("RSSI=-%d (dBm)\r\n",ssv6xxx_get_rssi_by_mac((u8 *)ap_list[i].bssid.addr,0));
            }
            else if((connected_info.vif_operate[1]==SSV6XXX_HWM_STA)&&
                   (!memcmp((void *)ap_list[i].bssid.addr,(void *)connected_info.vif_u[1].station_status.apinfo.Mac,ETHER_ADDR_LEN)))
            {
                    LOG_PRINTF("RSSI=-%d (dBm)\r\n",ssv6xxx_get_rssi_by_mac((u8 *)ap_list[i].bssid.addr,1));
            }
            else
            {
...
void cmd_ctl(s32 argc, char *argv[])
{
	...
	else if (strcmp(argv[1], "status")==0)
    {
        ...
        for(i=0;i<MAX_VIF_NUM;i++)
        {
            if((SSV6XXX_HWM_STA==info.vif_operate[i])||(SSV6XXX_HWM_SCONFIG==info.vif_operate[i]))
            {
                ...
                MEMCPY((void*)ssid_buf,(void*)info.vif_u[i].station_status.ssid.ssid,info.vif_u[i].station_status.ssid.ssid_len);
                LOG_PRINTF("SSID:%s\r\n",ssid_buf);
                ...
                if(is_valid_ether_addr(info.vif_u[i].station_status.apinfo.Mac)){
            ...
            else if(SSV6XXX_HWM_AP==info.vif_operate[i])
            {
                ...
                MEMCPY((void*)ssid_buf,(void*)info.vif_u[i].ap_status.ssid.ssid,info.vif_u[i].ap_status.ssid.ssid_len);
                LOG_PRINTF("SSID:%s\r\n",ssid_buf);
    ...
    else if (strcmp(argv[1], "ap")==0&&argc >= 3)
    {
        ...
        switch(argc)
        {
            ...
            case 6: 
                ...
                if((strcmp(argv[2], "on") == 0)&&(strcmp(argv[4], "vif") == 0)&&
                    (strlen(argv[3])<=MAX_SSID_LEN)&&(MAX_VIF_NUM>strtol(argv[5],NULL,10)))
                {
                    errormsg =FALSE;
                    ap.status = TRUE;
                    ap.vif_idx= (u8)strtol(argv[5],NULL,10);
                    ap.security = SSV6XXX_SEC_NONE;
                    ap.ssid.ssid_len = strlen(argv[3]);
                    //ap.channel = (u8)strtol(argv[5],NULL,10);
                    OS_MemSET((void *) ap.ssid.ssid,0,sizeof(ap.ssid.ssid));
                    MEMCPY( (void*)ap.ssid.ssid, (void*)argv[3], strlen(argv[3]));
                    LOG_PRINTF("vif:%d, SSID:%s,SSID len=%d, ch:%d. \r\n",ap.vif_idx,ap.ssid.ssid,strlen(argv[3]),ap.channel );
                }
                else if(strcmp(argv[2], "on") == 0&&(strlen(argv[3])<=MAX_SSID_LEN))
                {
            ...
            case 7:                 
                ...
                if(strcmp(argv[2], "on") == 0&&(MAX_VIF_NUM>strtol(argv[6],NULL,10))&& (strcmp(argv[5], "vif") == 0)&&
                        (strtol(argv[4],NULL,10)>0)&&(strlen(argv[3])<=MAX_SSID_LEN))
                {
                    errormsg =FALSE;
                    ap.status = TRUE;
                    ap.vif_idx= (u8)strtol(argv[6],NULL,10);
                    ap.ssid.ssid_len = strlen(argv[3]);
                    MEMCPY( (void*)ap.ssid.ssid, (void*)argv[3], strlen(argv[3]));
                    ap.channel = (u8)strtol(argv[4],NULL,10);
                }
                else if(strcmp(argv[2], "on") == 0&&(0<strtol(argv[4],NULL,10))&&
                        /*(strtol(argv[4],NULL,10)<=14)&&*/(strlen(argv[3])<=MAX_SSID_LEN))
                {
            ...
            case 8:
                ...
                if(strcmp(argv[2], "on") == 0&&(MAX_VIF_NUM>strtol(argv[7],NULL,10))&& 
                    (strcmp(argv[6], "vif") == 0)&&(strlen(argv[3])<=MAX_SSID_LEN))
                {
                    errormsg =FALSE;
                    ap.status = TRUE;
                    ap.vif_idx= (u8)strtol(argv[7],NULL,10);
                    ap.ssid.ssid_len = strlen(argv[3]);
                    MEMCPY( (void*)ap.ssid.ssid, (void*)argv[3], strlen(argv[3]));
                    //ap.channel = (u8)strtol(argv[5],NULL,10);
            ...
            case 9:                
                ...
                if(strcmp(argv[2], "on") == 0&&(MAX_VIF_NUM>strtol(argv[8],NULL,10))&& 
                    (strcmp(argv[7], "vif") == 0)&&(strlen(argv[3])<=MAX_SSID_LEN))
                {
                    errormsg =FALSE;
                    ap.status = TRUE;
                    ap.vif_idx= (u8)strtol(argv[8],NULL,10);
                    ap.ssid.ssid_len = strlen(argv[3]);
                    MEMCPY( (void*)ap.ssid.ssid, (void*)argv[3], strlen(argv[3]));
                    ap.channel = (u8)strtol(argv[4],NULL,10);
	...
	{   
        ...
        Sta_setting sta;
        u8 arg_cnt = 2,vif_idx;
        MEMSET(&sta, 0 , sizeof(Sta_setting));

        if(strcmp(argv[arg_cnt], "on") == 0)
        {
            if((argc>(arg_cnt+1))&&(strcmp(argv[arg_cnt+1], "vif") == 0))
            {
                arg_cnt=4; //x
                vif_idx = (u8)ssv6xxx_atoi(argv[arg_cnt]);
            ...
            {
                arg_cnt++;
                if(strcmp(argv[arg_cnt], "static")==0)
                {
                    ...
                    netmgr_dhcpc_set(false,vif_idx);
                    {
                        ...
                        info.ipv4 = netstack_inet_addr("192.168.1.100");
                        info.netmask = netstack_inet_addr("255.255.255.0");
                        info.gateway = netstack_inet_addr("192.168.1.1");
                        info.dns = netstack_inet_addr("192.168.1.1");
                
                        netmgr_ipinfo_set(vif_idx, &info, false);
                    }
                }
            }            
            else
...
void cmd_ampdu(s32 argc, char *argv[])
{
    if (argc == 3)
    {
        ...
        else if (strcmp(argv[1], "txdtime")==0)
        {
            mode = AMPDU_TX_OPT_SET_FORCE_DELAY_TIME;
            LOG_PRINTF("AMPDU tx force delay time = %d(us)\r\n", value*100);
        }
