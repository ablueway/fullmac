...
void FAST_FUNC load_config(struct server_config_t* psconfig)
{
    ...
    struct ip_addr ipaddr, subnet, gw, dns;
	if (psconfig == NULL)
		return;

	OS_MemSET(psconfig, 0, sizeof(struct server_config_t));
...
int FAST_FUNC dhcps_set_info_api(dhcps_info *if_dhcps,char *if_name)
{
    ipinfo info;
    u32 softap_ip = 0;
    ...
    netmgr_ipinfo_get(if_name, &info);
    ipaddr.addr = info.ipv4;
