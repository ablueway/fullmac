...
ALIGN_ARRAY(4) char bb_common_bufsiz1[COMMON_BUFSIZE];
 ...
 */
#ifdef ARM_ADS
const u8 dhcp_option_lengths[] = {
    0,//NO use
	4,//[OPTION_IP] =      4,
	8,//[OPTION_IP_PAIR] = 8,
	1,//[OPTION_STRING] =  1,  /* ignored by udhcp_str2optset */
	1,//[OPTION_STRING_HOST] = 1,  /* ignored by udhcp_str2optset */
    //  [OPTION_BOOLEAN] = 1,
	1,//[OPTION_U8] =      1,
	2,//[OPTION_U16] =     2,
    //[OPTION_S16] =     2,
	...
	5,//[OPTION_STATIC_ROUTES] = 5,
	22,//[OPTION_6RD] =    22,  /* ignored by udhcp_str2optset */
#if ENABLE_FEATURE_UDHCP_RFC3397
    1,//[OPTION_DNS_STRING] = 1,  /* ignored by both udhcp_str2optset and xmalloc_optname_optval */
    1,//[OPTION_SIP_SERVERS] = 1,
#endif
};
#else
...
static void log_option(const char *pfx, const uint8_t *opt)
{
	if (dhcp_verbose >= 2) {
		...
		*bin2hex(buf, (void*) (opt + OPT_DATA), opt[OPT_LEN]) = '\0';
		bb_info_msg("%s: 0x%02x %s\r\n", pfx, opt[OPT_CODE], buf);
	}
}
#else
...
void udhcp_printf(const char *fmt, ...)
{
    va_list args;
    char    p[128] = {0};
...
uint8_t* FAST_FUNC udhcp_get_option(struct dhcp_packet *packet, int code)
{
	...
	optionptr = (uint8_t *)packet->options;
	...
	while (1) {
		...
		if (optionptr[OPT_CODE] == DHCP_END) {
			...
			if ((overload & SNAME_FIELD101) == SNAME_FIELD) {
				...
				optionptr = (uint8_t *)packet->sname;
...
{
	unsigned len;
	uint8_t *optionptr = (uint8_t *)packet->options;
...
//TODO: learn how to use overflow option if we exhaust packet->options[]
	...
	OS_MemCPY((void*)(optionptr + end), addopt, len);
...
void FAST_FUNC udhcp_add_simple_option(struct dhcp_packet *packet, uint8_t code, uint32_t data)
{
	...
	bb_error_msg("can't add option 0x%02x\r\n", code);
}
		...
		int length)
{
	...
	if (!existing) {
		struct option_set *new_opt, **curr;

		/* make a new option */
		log2("Attaching option %02x to list", optflag->code);
		new_opt = (struct option_set *)OS_MemAlloc(sizeof(*new_opt));
		new_opt->data = (u8 *)OS_MemAlloc(length + OPT_DATA);
		new_opt->data[OPT_CODE] = optflag->code;
		new_opt->data[OPT_LEN] = length;
		OS_MemCPY((void*)(new_opt->data + OPT_DATA), (void*)(allocated ? allocated : buffer), length);
	...
	if (optflag->flags & OPTION_LIST) {
		...
		if (old_len + length < 255) {
            ...
            OS_MemFree((void *)existing->data);
			existing->data = (u8 *)OS_MemAlloc(OPT_DATA + 1 + old_len + length);
			...
			OS_MemCPY((void*)(existing->data + OPT_DATA + old_len), (void*)(allocated ? allocated : buffer), length);
			existing->data[OPT_LEN] = old_len + length;
		} /* else, ignore the data, we could put this in a second option in the future */
	} /* else, ignore the new data */

 ret:
 	 OS_MemFree((void*)allocated);
}
#endif

int FAST_FUNC udhcp_str2optset(const char *const_str, void *arg)
{
	UDHCP_DEBUG("udhcp_str2optset %s %p\r\n", const_str, arg);
	return 0;
#if 0
