...
{
    { "dhcpd_task",  (OsMsgQ)0, 4, OS_DHCPD_TASK_PRIO, DHCPD_STACK_SIZE, NULL, udhcpd_start_task   },
};
    ...
    *count = 0;
	...
	for (i = 0; i < server_config.max_leases; i++)
    {
		if (g_leases[i].lease_nip != 0)
		{
           ...
           OS_MemCPY((void*)(lease + *count), (void*)(&g_leases[i]), sizeof(struct dyn_lease));
           (*count)++;
		}
	}
...
static void send_packet_to_client(int s, struct dhcp_packet *dhcp_pkt, int force_broadcast)
{
    ...
    OS_MemSET((void*)&dest_addr, 0, sizeof(dest_addr));
	...
	OS_MemCPY((void*)dest_sin, (void*)&dest_addr, dest_length);
	...
	OS_MemSET((void*)&pdhcp_pkt, 0, DHCP_SIZE);
	OS_MemCPY((void*)&pdhcp_pkt , (void*)dhcp_pkt, dhcp_pkt_len);
...
static void init_packet(struct dhcp_packet *packet, struct dhcp_packet *oldpacket, char type)
{
	...
	OS_MemCPY((void*)packet->chaddr, (void*)oldpacket->chaddr, sizeof(oldpacket->chaddr));
...
static void add_server_options(struct dhcp_packet *packet)
{
	...
	if (server_config.sname)
        ...
        OS_MemCPY((void*)packet->sname,(void*)server_config.sname,sizeof(packet->sname) - 1);
	if (server_config.boot_file)
		//strncpy((char*)packet->file, server_config.boot_file, sizeof(packet->file) - 1);
        OS_MemCPY((void*)packet->file,(void*)server_config.boot_file,sizeof(packet->file) - 1);
}
...
void  udhcpd_run(void *arg)
{
    ...
    if (dest_sin)
    {
        OS_MemSET((void *) dest_sin, 0, sizeof(struct sockaddr));
    }
    ...
    OS_MemSET((char *)g_leases, 0, server_config.max_leases * sizeof(g_leases[0]));
	...
	if (server_socket < 0){
        ...
        OS_MemSET((char *)packet, 0, sizeof(struct dhcp_packet));
		while (dhcpd_run)
        {
			...
			if (recv_size < 0){
				...
				if (static_lease_nip) {
					...
					OS_MemCPY((void*)&fake_lease.lease_mac, (void*)packet->chaddr, 6);
				...
				switch (state[0]) {
					...
					case DHCPDECLINE:
						...
						if (server_id_opt
							...
							OS_MemSET((void*)lease->lease_mac, 0, sizeof(lease->lease_mac));
...
void  udhcpd_start_task(void *arg)
{
    ...
    for(;;)
    {
        res = msg_evt_fetch_timeout(mbox, &msg_evt, 0);
        if (res != OS_SUCCESS)
        {
            //UDHCP_DEBUG("Could not receive from the queue!\r\n");
        }
        /* Start dhcpd*/
        else if (msg_evt && (msg_evt->MsgData == 1)){
...
int udhcpd_init_global_parms(void)
{
    if (dest_sin == NULL)
    {
...
{
	if (udhcpd_init_global_parms() != 0)
	{
		UDHCP_DEBUG("udhcpd init global parms fail!!\r\n");
		return -1;
	}
    if (OS_MsgQCreate(&st_dhcpd_task[0].qevt, (s32)st_dhcpd_task[0].qlength) != OS_SUCCESS)
    {
...
int udhcpd_start(void)
{
    ...
    if (dhcpd_start == false)
    {
        ...
        msg_evt->MsgData = 1;
        msg_evt_post(st_dhcpd_task[0].qevt, msg_evt);
...
int udhcpd_stop(void)
{
    ...
    if (dhcpd_start == true)
    {
        ...
        udhcp_send_kernel_packet_dummy("STOP", strlen("STOP"),
                htonl(0x7f000001), SERVER_PORT);
