...
void WIFImode_Handler(char *pcInsert)
{
    ...
    ret = netmgr_wifi_mode_get(&mode, &status, 0);
...
void SSID_Handler(char *pcInsert)
{
    ...
    ret = netmgr_wifi_info_get(info,0);

    if ((SSV6XXX_HWM_STA == info->vif_operate[0] ) && (info->status == true))
    {
        //STRCPY(pcInsert, (void*)info->u.station.ssid.ssid);
        MEMCPY((void*)pcInsert, (void*)info->vif_u[0].station_status.ssid.ssid,info->vif_u[0].station_status.ssid.ssid_len);
    }
    else if ((SSV6XXX_HWM_AP == info->vif_operate[0] ) && (info->status == true))
    {
        //STRCPY(pcInsert, (void*)info->u.ap.ssid.ssid);
        MEMCPY((void*)pcInsert, (void*)info->vif_u[0].ap_status.ssid.ssid,info->vif_u[0].ap_status.ssid.ssid_len);
    }
    else if ((SSV6XXX_HWM_SCONFIG == info->vif_operate[0] ) && (info->status == true))
    {
        //STRCPY(pcInsert, (void*)info->u.station.ssid.ssid);
        MEMCPY((void*)pcInsert, (void*)info->vif_u[0].station_status.ssid.ssid,info->vif_u[0].station_status.ssid.ssid_len);
    }
...
void Encrypt_Handler(char *pcInsert)
{
    ...
    if ((SSV6XXX_HWM_STA == info->vif_operate[0]) && (info->status == true))
    {
        if (info->vif_u[0].station_status.capab_info&BIT(4))
        {
            STRCPY(pcInsert, info->vif_u[0].station_status.proto&WPA_PROTO_WPA?"WPA":
                (info->vif_u[0].station_status.proto&WPA_PROTO_RSN?"WPA2":"WEP"));
        }
        else
        {
    ...
    {
        if(info->vif_u[0].ap_status.key_mgmt == WPA_KEY_MGMT_NONE)
        {
            if(info->vif_u[0].ap_status.pairwise_cipher == WPA_CIPHER_NONE)
                STRCPY(pcInsert, "OPEN");
            else
                STRCPY(pcInsert, "WEP");
        }
...
void IPmode_Handler(char *pcInsert)
{
    ...
    ret = netmgr_dhcp_status_get(&dhcpd_status, &dhcpc_status, 0);
    if (dhcpd_status)
    {
...
void Channel_Handler(char *pcInsert)
{
    ...
    if (ret == 0)
    {
        if ((SSV6XXX_HWM_STA == info->vif_operate[0] ) && (info->status == true))
        {
            sprintf(pcInsert, "%d", info->vif_u[0].station_status.channel);
        }
        else if ((SSV6XXX_HWM_AP == info->vif_operate[0] ) && (info->status == true))
        {
            sprintf(pcInsert, "%d", info->vif_u[0].ap_status.channel);
        }
        else if ((SSV6XXX_HWM_SCONFIG == info->vif_operate[0] ) && (info->status == true))
        {
            //sprintf(pcInsert, "");
