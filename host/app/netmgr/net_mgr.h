...
extern int netmgr_wifi_mode_get(wifi_mode *mode, bool *status, u8 vif_idx);
extern int netmgr_wifi_info_get(Ap_sta_status *info, u8 vif_idx);
extern void netmgr_cfg_get(netmgr_cfg *p_cfg,u8 vif_idx);
extern void netmgr_cfg_set(netmgr_cfg *p_cfg,u8 vif_idx);
...
extern int netmgr_ipinfo_set(u8 vif_idx, ipinfo *info, bool auto_dhcpd_info);
extern int netmgr_hwmac_get(char *ifname, void *mac);
extern bool netmgr_wifi_is_connected(ssv6xxx_hw_mode hmode);
extern void netmgr_netif_status_set(bool on,u8 vif_idx);
/* TODO :aaron */
//extern bool netmgr_netif_status_get();
extern int netmgr_dhcps_info_set(dhcps_info *if_dhcps,u8 vif_idx);
extern int netmgr_dhcpc_set(bool dhcpc_enable, u8 vif_idx);
extern int netmgr_dhcpd_set(bool dhcpd_enable, u8 vif_idx);
extern int netmgr_dhcp_status_get(bool *dhcpd_status, bool *dhcpc_status, u8 vif_idx);
extern int netmgr_dhcd_ipmac_get(dhcpdipmac *ipmac, int *size_count);
extern bool netmgr_wifi_check_mac(unsigned char * mac);
extern bool netmgr_wifi_check_sconfig(u8* vif_idx);
extern bool netmgr_wifi_check_sta(u8* vif_idx);
extern bool netmgr_wifi_check_ap(u8* vif_idx);
/* TODO :aaron */
//extern bool netmgr_wifi_check_ap_ready();
extern bool netmgr_wifi_check_sta_connected(u8* vif_idx);
extern void netmgr_netif_link_set(bool on,u8 vif_idx);
...
extern int netmgr_wifi_leave_async(u8 vif_idx);
...
extern int netmgr_wifi_leave(u8 vif_idx);
extern int netmgr_wifi_control(wifi_mode mode, wifi_ap_cfg *ap_cfg, wifi_sta_cfg *sta_cfg);
extern void netmgr_wifi_station_off(u8 vif_idx);
extern void netmgr_wifi_ap_off(u8 vif_idx);
...
extern u32 g_sta_5g_channel_mask;
...
extern int netmgr_wifi_scan_ex_async(u16 channel_mask, u32 channel_5g_mask, char *ssids[], int ssids_count);
extern int netmgr_wifi_scan_ex(u16 channel_mask, u32 channel_5g_mask, char *ssids[], int ssids_count);
