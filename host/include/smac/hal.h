...
#define HAL_SET_MRX_FLT(_sh, _regval)       _sh->hal_ops.set_mrx_flt(_sh, _regval)
#define HAL_GET_MRX_FLT(_sh, _regval)       _sh->hal_ops.get_mrx_flt(_sh, _regval)
#endif
...
#define HAL_RATE_REPORT_HANDLER(_sc, _skb)								  \
					_sc->sh->hal_ops.rate_report_handler(_sc, _skb)
...
#define HAL_HT_RATE_UPDATE(_sc, _skb, _rates)										  \
					_sc->sh->hal_ops.rc_ht_update_update(_skb, _sc, _rates)
...
#define HAL_EDCA_STAT(_sh, _primary, _secondary)        _sh->hal_ops.edca_stat(_sh, _primary, _secondary)
...
#define HAL_UPDATE_RSSI(_sc, _skb, _rxs)                                              \
                _sc->sh->hal_ops.update_rssi(_sc, _skb, _rxs)
...
#define HAL_AMPDU_MAX_TRANSMIT_LENGTH(_sc, _skb, _rate_idx) 								  \
				_sc->sh->hal_ops.ampdu_max_transmit_length(_skb, _rate_idx)
