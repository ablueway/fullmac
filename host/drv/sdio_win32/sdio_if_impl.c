...
//  < 0 : fail
// >= 0 : # of bytes recieve
s32 	recv_sdio_impl(u8 *dat, size_t len)
{
...
s32 	send_sdio_impl(void *dat, size_t len)
{
	SDIO_TRACE("%s <= \n", __FUNCTION__);
...
bool	get_name_sdio_impl(char name[32])
{
	SDIO_TRACE("%s <= : name = %s\n", __FUNCTION__, name);
...
bool	ioctl_sdio_impl(u32 ctl_code, 
...
const struct ssv6xxx_drv_ops	g_drv_sdio =
{
	...
	sdio_init,
	recv_sdio_impl,
	sdio_write_data,	// send_sdio_impl, 
	get_name_sdio_impl,
	ioctl_sdio_impl,
	sdio_handle,
    sdio_ack_int,
