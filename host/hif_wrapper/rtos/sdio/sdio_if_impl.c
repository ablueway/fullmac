...
static ALIGN_ARRAY(4) u8 sdio_data_align[MAX_DATA_BYTE];
#endif

bool sdio_if_write_reg(u32 addr, u32 data);
...
size_t sdio_get_readsz(bool bAgg,size_t next_len)
{
    s32 ret;
    u32 val=0;
    u8 err=0;
    ...
    if (!bAgg)
    {
        ...
        if(!next_len)
        {
            ...
            ret = sdio_read_cmd53(dataIOPort, (void*)&rlen, sizeof(size_t));
            if (ret == false) { 
...
bool _sdio_if_read_reg(u32 addr , u32 *data);

s32 sdio_if_recv_data(u8 *dat, size_t len)
{
    size_t rlen;
    s32 ret;
    u8 int_status;
    ...
    if ((len >= MAX_DATA_BYTE)&&(!g_host_cfg.hci_rx_aggr))
    {
    ...
    {
        void* tmp_addr=NULL;
        tmp_addr = (u8*)OS_MemAlloc(rlen+64);
        if(tmp_addr)
...
s32	sdio_if_write_data(void *dat, size_t len)
{
    s32 ret = 0;
    int retry = 10;
...
bool sdio_if_write_sram(u32 addr, u8 *data, u32 size)
{
    s32 ret;
...
u32 sdio_if_write_fw_to_sram(u8* fw_bin, u32 fw_bin_len, u32 block_size)
{
    u8   *fw_buffer = NULL;
    u8   *fw_ptr = fw_bin;
    ...
    while(bin_len > 0){
        ...
        if (1)
        {
            u32 data32 = 0, reg32 = 0;
            for (i = 0; i < block_size / 4; i ++)
            {
                data32 = *((u32 *)(&fw_buffer[i * 4]));
...
s32 sdio_if_start(void)
{
    return TRUE;
}

s32 sdio_if_stop(void)
{
    return TRUE;
}

bool sdio_if_wakeup_wifi(bool sw)
{
    bool ret = 0;
    if(sw)
    {
...
bool sdio_if_detect_card(void)
{
    return sdio_host_detect_card();
}

bool sdio_if_writeb(u8 func,u32 addr, u8 data)
{
    bool ret = 0;
...
u32 sdio_if_readb(u8 func,u32 addr)
{
    u8 data=0;
