...
struct ssv6xxx_sdio_tuning_glue     *glue=NULL;
...
static INTN ssv6xxx_sdio_tuning_cmd52_read(OS_WRAPPER_SDIO_FUNC *func, UINT32 addr, UINT8 *value)
{
    ...
    ret=OS_WRAPPER_SDIO_CMD52_RD(func, addr, &data);
    *value=data;
...
static INTN ssv6xxx_sdio_tuning_read_register(OS_WRAPPER_SDIO_FUNC *func, UINT32 addr, UINT32 *buf)
{
    ...
    do
    {
        OS_WRAPPER_SDIO_CMD52_RD(func, 0xf, &val);
...
static INTN ssv6xxx_sdio_tuning_onebit_mode(struct ssv6xxx_sdio_tuning_glue *glue, OS_WRAPPER_SDIO_FUNC *func)
{
    ...
    /* set one bit mode*/
    OS_WRAPPER_SDIO_SET_BUS_WIDTH(func, MMC_BUS_WIDTH_1);
...
static INTN ssv6xxx_sdio_tuning_output_data(struct ssv6xxx_sdio_tuning_glue *glue, OS_WRAPPER_SDIO_FUNC *func)
{
    ...
    /* set four bit mode*/
    OS_WRAPPER_SDIO_SET_BUS_WIDTH(func, MMC_BUS_WIDTH_4);
...
INTN ssv6xxx_sdio_tuning_initialization(void *func, const void *id)
{
    INTN ret = 0;
    ...
    glue = OS_WRAPPER_MEM_ALLOC(sizeof(struct ssv6xxx_sdio_tuning_glue), 0);
    if (!glue) {
    ...
    OS_WRAPPER_SDIO_SET_CLK(func, HIGH_SPEED_SDIO_CLOCK);//MID_SPEED_SDIO_CLOCK //HIGH_SPEED_SDIO_CLOCK
...
void ssv6xxx_sdio_tuning_deinitialization(void *func)
{
#if 0
    struct ssv6xxx_sdio_tuning_glue *glue = sdio_get_drvdata(func);

    OS_WRAPPER_LOG("ssv6xxx_sdio_tuning_remove..........\n");
    if (glue)
