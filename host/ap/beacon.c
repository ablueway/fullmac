...
static u8 * hostapd_eid_country_add(u8 *pos, u8 *end, int chan_spacing,u8 s,u8 e)
{
	if (end - pos < 3)
		return pos;
...
//start :modify by angel.chiu for wpa
    if(pApInfo->ap_conf.wpa == WPA_PROTO_RSN)
    {
        const u8 *ie;
    	size_t ielen;
    	ie = (const u8 *)wpa_auth_get_wpa_ie(&(pApInfo->wpa_auth), &ielen);
...
u8 *ieee80211_beacon_add_tim(ApInfo_st *pApInfo,u8 *eid)
{
    u8 *pos, *tim;
    int aid0 = 0;
...
void GenBeacon()
{
	...
	ap_soc_set_bcn(SSV6XXX_SET_BEACON, gDeviceInfo->APInfo->bcn, &gDeviceInfo->APInfo->bcn_info, AP_DEFAULT_DTIM_PERIOD-1, g_host_cfg.bcn_interval);
