...
static const u32 eapol_key_timeout_first = 100;//100; /* ms */
static const u32 eapol_key_timeout_subseq = 1000;//1000; /* ms */
static const u32 eapol_key_timeout_error = 100;
...
{	
	os_cancel_timer(wpa_send_eapol_timeout, wpa_auth, sm);	
}
...
{    
   ...
   MEMCPY(pApInfo->wpa_psk.psk,&(pmk_value->u.pmk[0]),PMK_LEN);
   Send_Beacon();
   
}

static ssv6xxx_data_result wpa_receive_cb(void *frame,u32 len)
{
...
void ssv6xxx_eapol_data_reg_cb()
{
...
static int ssv6xxx_set_key( enum wpa_alg alg, const u8 *addr,
				      ...
				      const u8 *key, size_t key_len)
{
    ...
    MEMCPY(sec_entry.bssid,(void *)addr,6);
    MEMCPY(sec_entry.key,(void *)key, key_len); 
	...
	size_t data_len, int encrypt)
{
	...
	MEMCPY((void *)hdr->IEEE80211_DA_FROMDS, addr, ETH_ALEN);
	MEMCPY((void *)hdr->IEEE80211_BSSID_FROMDS, gDeviceInfo->APInfo->own_addr, ETH_ALEN);
	MEMCPY((void *)hdr->IEEE80211_SA_FROMDS, gDeviceInfo->APInfo->own_addr, ETH_ALEN);
	pos = (u8 *) ((hdr->IEEE80211_SA_FROMDS)+ETH_ALEN+2);
...
int wpa_auth_derive_psk(ApInfo_st *pApInfo)
{
	...
	MEMSET((void *)&apcfg,0x0,sizeof( struct ap_calc_pmk_cfg));
	wpa_hexdump_ascii(MSG_DEBUG, "SSID",
			  (u8 *) pApInfo->config.ssid,pApInfo->config.ssid_len);
	wpa_hexdump_ascii_key(MSG_DEBUG, "PSK (ASCII passphrase)",
			      (u8 *) pApInfo->password,
			      STRLEN((void *)pApInfo->password));
	
   
    apcfg.ssid_len = pApInfo->config.ssid_len;
    MEMCPY(apcfg.ssid,pApInfo->config.ssid,apcfg.ssid_len);
    MEMCPY((void *)apcfg.password,  pApInfo->password, STRLEN((void *)pApInfo->password));
    _ssv6xxx_wifi_ioctl_Ext(SSV6XXX_HOST_CMD_GET_PMK, &apcfg, sizeof(struct ap_calc_pmk_cfg), TRUE, FALSE);
...
void wpa_get_ntp_timestamp(u8 *buf)
{
	...
	now = os_tick2ms(os_sys_jiffies());
	sec = now/MSEC_PER_SEC + 2208988800U; /* Epoch to 1900 */
	/* Estimate 2^32/10^6 = 4295 - 1/32 - 1/512 */
	usec = now-((now/MSEC_PER_SEC)*MSEC_PER_SEC);
			       ...
			       const u8 *addr)
{
    ...
    if(sta !=NULL)
    {
         ...
         os_cancel_timer(ap_handle_timer,  (u32)gDeviceInfo->APInfo, (u32)sta);
					  ...
					  struct wpa_group *group)
{
	...
	//	return -1;
	MEMCPY(buf,gDeviceInfo->APInfo->own_addr, ETH_ALEN);
...
{  
	//MEMCPY(wpa_auth->addr, gDeviceInfo->APInfo->own_addr, ETH_ALEN);
...
void wpa_auth_sta_init(struct wpa_authenticator *wpa_auth, const u8 *addr,struct wpa_state_machine *sm)
{
     
    MEMSET(sm,0x0,sizeof(struct wpa_state_machine));
...
void wpa_auth_sta_deinit(struct wpa_state_machine *sm)
{
	...
	os_cancel_timer(wpa_send_eapol_timeout, sm->wpa_auth, sm);
		 ...
		 u8 *data, size_t data_len)
{
	...
	if (key_info & WPA_KEY_INFO_REQUEST) {
		if (sm->MICVerified) {
			...
			MEMCPY((void *)sm->req_replay_counter,(void *) key->replay_counter,
				  WPA_REPLAY_COUNTER_LEN);
		} else {
			wpa_auth_logger(wpa_auth, sm->addr, LOGGER_INFO,
					"received EAPOL-Key request with "
		...
		if (msg == SMK_ERROR) {
			...
			wpa_auth_logger(wpa_auth, sm->addr, LOGGER_INFO,
					...
					"rekeying");
            //LOG_DEBUG("++++++++++++++cancel timer\n\r");
			os_cancel_timer(wpa_rekey_gtk, wpa_auth, NULL);
	...
	MEMCPY((void *)sm->SNonce, (void *)key->key_nonce, WPA_NONCE_LEN);
	wpa_sm_step(sm);
}
		      ...
		      int keyidx, int encr, int force_version)
{
	...
	MEMCPY((void *)key->replay_counter,(void *)sm->key_replay[0].counter,
		  WPA_REPLAY_COUNTER_LEN);
	sm->key_replay[0].valid = TRUE;

	if (nonce)
    {   
		MEMCPY((void *)key->key_nonce, nonce, WPA_NONCE_LEN);
        wpa_hexdump_key(MSG_DEBUG,"ANONCE:",nonce,WPA_NONCE_LEN);
    }
	if (key_rsc)
		MEMCPY((void *)key->key_rsc, key_rsc, WPA_KEY_RSC_LEN);
    
	if (kde && !encr) {
		MEMCPY((void *)(key + 1),(void *) kde, kde_len);
		...
		if (version == WPA_KEY_INFO_TYPE_HMAC_SHA1_AES ||
			...
			u8 ek[32];
			MEMCPY((void *)key->key_iv,
				  sm->group->Counter + WPA_NONCE_LEN - 16, 16);
			inc_byte_array(sm->group->Counter, WPA_NONCE_LEN);
			MEMCPY((void *)ek, (void *)key->key_iv, 16);
			MEMCPY((void *)(ek + 16),(void *)sm->PTK.kek, 16);
			MEMCPY((void *)(key + 1), (void *)buf, key_data_len);
			rc4_skip((void *)ek, 32, 256, (u8 *) (key + 1), key_data_len);
			WPA_PUT_BE16(key->key_data_length, key_data_len);
		}
		FREE(buf);
	}
	...
	if (key_info & WPA_KEY_INFO_MIC) {
		...
		wpa_eapol_key_mic((void *)sm->PTK.kck, version, (u8 *) hdr, len,
				  (void *)key->key_mic);
	}
			   ...
			   int keyidx, int encr)
{
	...
	wpa_printf(MSG_DEBUG, "WPA: Use EAPOL-Key timeout of %u ms (retry "
		   "counter %d)\n\r", timeout_ms, ctr);  
    os_cancel_timer(wpa_send_eapol_timeout, wpa_auth);
...
static int wpa_verify_key_mic(struct wpa_ptk *PTK, u8 *data, size_t data_len)
{
	...
	MEMCPY((void *)mic, (void *)key->key_mic, 16);
	MEMSET((void *)key->key_mic, 0, 16);
    startCount = os_getSysTime;
	if (wpa_eapol_key_mic((u8 *)PTK->kck, key_info & WPA_KEY_INFO_TYPE_MASK,
			      data, data_len, (u8 *)key->key_mic) ||
	...
	MEMCPY((void *)key->key_mic, (void *)mic, 16);
	return ret;
}
...
void wpa_remove_ptk(struct wpa_state_machine *sm)
{
	...
	{	     
	    MEMSET((void *)(&sm->PTK), 0, sizeof(sm->PTK));
			  ...
			  struct wpa_ptk *ptk)
{
	...
	if (MEMCMP(gDeviceInfo->APInfo->own_addr,sm->addr , ETH_ALEN) < 0) {
		MEMCPY(data,gDeviceInfo->APInfo->own_addr, ETH_ALEN);
		MEMCPY(data + ETH_ALEN, sm->addr, ETH_ALEN);
	} else {
		MEMCPY(data, sm->addr, ETH_ALEN);
		MEMCPY(data + ETH_ALEN, gDeviceInfo->APInfo->own_addr, ETH_ALEN);
	}

	if (MEMCMP(sm->ANonce, sm->SNonce, WPA_NONCE_LEN) < 0) {
		MEMCPY(data + 2 * ETH_ALEN, sm->ANonce, WPA_NONCE_LEN);
		MEMCPY(data + 2 * ETH_ALEN + WPA_NONCE_LEN, sm->SNonce,
			  WPA_NONCE_LEN);
	} else {
		MEMCPY(data + 2 * ETH_ALEN, sm->SNonce, WPA_NONCE_LEN);
		MEMCPY(data + 2 * ETH_ALEN + WPA_NONCE_LEN, sm->ANonce,
			  WPA_NONCE_LEN);
	}
    

    sha1_prf(pmk, PMK_LEN,  "Pairwise key expansion", data, sizeof(data), (u8 *)ptk,
        			 ptk_len);
...
SM_STATE(WPA_PTK, PTKCALCNEGOTIATING)
{
	...
	for (;;) {
		...
		if (wpa_verify_key_mic((struct wpa_ptk *)&PTK, (u8 *)sm->last_rx_eapol_key,
	...
	MEMCPY((void *)(&sm->PTK), (void *)&PTK, sizeof(PTK));
	sm->PTK_valid = TRUE;
