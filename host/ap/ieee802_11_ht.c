...
u8 * hostapd_eid_ht_capabilities(ApInfo_st *pApInfo, u8 *eid)
{
	...
	if (!pApInfo->config.b80211n|| !pApInfo->eCurrentApMode||
	    pApInfo->config.disable_11n)
		return eid;
	...
	*pos++ = sizeof(struct ieee80211_ht_capabilities);
    ...
    MEMCPY((void*)pos, (void*)&host_to_le16(ht_capab), 2);
    
	cap->a_mpdu_params = pApInfo->config.a_mpdu_params;
        
	MEMCPY((void*)cap->supported_mcs_set, pApInfo->config.mcs_set,16);
...
u8 * hostapd_eid_ht_operation(ApInfo_st *pApInfo, u8 *eid)
{
	...
	*pos++ = sizeof(struct ieee80211_ht_operation);
	...
	MEMSET((void *)oper, 0, sizeof(*oper));
    ...
    if(IS_40MHZ_AVAILABLE())
    {
        ...
        if(-1!=i)
        {
            switch(ssv6xxx_5ghz_chantable[i].flags)
            {
    ...
    MEMCPY((void*)&oper->operation_mode, (void*)&host_to_le16(pApInfo->ht_op_mode), 2);
    pos += sizeof(struct ieee80211_ht_operation);
...
u16 copy_sta_ht_capab(ApInfo_st *pApInfo, APStaInfo_st *sta,
		      const u8 *ht_capab)
{
	/*
	...
	if (!ht_capab ||
        ...
        MEMSET((void *)&sta->ht_capabilities,0,sizeof(struct ieee80211_ht_capabilities));
		return WLAN_STATUS_SUCCESS;
	}

	sta->_flags |= WLAN_STA_HT;
	MEMCPY((void *)&sta->ht_capabilities, ht_capab,
