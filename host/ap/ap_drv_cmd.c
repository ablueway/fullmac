...
void ap_soc_set_bcn(enum ssv6xxx_tx_extra_type extra_type, void *frame, struct cfg_bcn_info *bcn_info, u8 dtim_cnt, u16 bcn_itv)
{
   ...
   {
#if (AUTO_BEACON != 0)
        ssv_hal_beacon_set(frame, bcn_info->tim_cnt_oft);
#else
        if (0==ssv_hal_beacon_set(frame, bcn_info->tim_cnt_oft))
        {
        ...
        {
            bool post_msg = false;//post message to cmd_engine
            OS_MutexLock(gDeviceInfo->APInfo->g_dev_bcn_mutex);
...
#endif
bool ap_sta_info_collect(struct cfg_set_sta *cfg_sta,enum cfg_sta_oper sta_oper, APStaInfo_st *sta, enum cfg_ht_type ht, bool qos)
{
    ...
    if(sta->addr&&sta->wsid_idx<gMaxAID)
    {
        int i = 0;
        ret = TRUE;
        MEMCPY((void*)&wsid_info->addr, (void*)sta->addr, ETH_ALEN);
        ...
        //mcs
        MEMCPY((void*)cfg_sta->ht_capabilities.supported_mcs_set,(void*)sta->ht_capabilities.supported_mcs_set,16);
        //non-ht-support rate
...
//check if write buffer too much
    ...
    #if(SW_8023TO80211==1)
    ssv_hal_fill_txreq0(frame,len,0,NULL,NULL,NULL,1,0,0);
    #endif
    TxHdl_FrameProc(frame, bAPFrame, 0, TxFlags, (ssv_vif*)gDeviceInfo->APInfo->vif);
	return 0;
}
#endif
