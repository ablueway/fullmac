...
#ifdef __AP_DEBUG__
	extern void APStaInfo_PrintStaInfo(u32 idx);
#endif//__AP_DEBUG__
			   ...
			   int iswep)
{
	...
	if (auth_transaction == 1) {
		if (!sta->challenge) {
			...
			int r, tick;
			sta->challenge = (u8*)MALLOC(WLAN_AUTH_CHALLENGE_LEN);
			 ...
			 int reassoc)
{
	...
	if (sta == NULL || test_sta_flag(sta, WLAN_STA_AUTH) == 0) {
        ...
        if(is_valid_ether_addr((const u8*)mgmt->sa))
...
// 		os_memcpy(sta->last_assoc_req, mgmt, len);
    ...
    if((gDeviceInfo->APInfo->sec_type==SSV6XXX_SEC_WEP_40)||(gDeviceInfo->APInfo->sec_type==SSV6XXX_SEC_WEP_104))
    {
        ...
        ssv_hal_ap_wep_setting(gDeviceInfo->APInfo->sec_type,gDeviceInfo->APInfo->password,
                               ((ssv_vif*)gDeviceInfo->APInfo->vif)->idx, (u8*)&sta->addr[0]);
    }
...
static s32 ap_mlme_rx_assoc_request(struct ApInfo*pApInfo, struct ieee80211_mgmt *mgmt, u16 len, u8 bssid_idx)
{
...
static s32 ap_mlme_rx_reassoc_request(struct ApInfo*pApInfo, struct ieee80211_mgmt *mgmt, u16 len, u8 bssid_idx)
{
...
static s32 ap_mlme_rx_probe_request(struct ApInfo*pApInfo, struct ieee80211_mgmt *mgmt, u16 len, u8 bssid_idx)
{
...
static s32 ap_mlme_rx_beacon(struct ApInfo*pApInfo, struct ieee80211_mgmt *mgmt, u16 len, u8 bssid_idx)
{
...
static s32 ap_mlme_rx_atim(struct ApInfo*pApInfo, struct ieee80211_mgmt *mgmt, u16 len,u8 bssid_idx)
{
...
static s32 ap_mlme_rx_disassoc(struct ApInfo*pApInfo, struct ieee80211_mgmt *mgmt, u16 len, u8 bssid_idx)
{
...
static s32 ap_mlme_rx_auth(struct ApInfo*pApInfo, struct ieee80211_mgmt *mgmt, u16 len, u8 bssid_idx)
{
...
static s32 ap_mlme_rx_deauth(struct ApInfo*pApInfo, struct ieee80211_mgmt *mgmt, u16 len, u8 bssid_idx)
{
	APStaInfo_st *sta;
...
static s32 ap_mlme_rx_action(struct ApInfo*pApInfo, struct ieee80211_mgmt *mgmt, u16 len, u8 bssid_idx)
{
	return AP_MLME_OK;
}
...
void AP_RxHandleAPMode(void *frame)
{
    ...
    if(1==ssv_hal_get_rxpkt_f80211(pPktInfo))
    {
        ...
        u32 nDataLen =ssv_hal_get_rxpkt_data_len(pPktInfo);
        u8 bssid_idx = ssv_hal_get_rxpkt_bssid_idx(pPktInfo);
        if ( WLAN_FC_GET_TYPE(fc) == WLAN_FC_TYPE_MGMT )
        {
            u8 stype;
    ...
    else
    {
        ...
        u8 *eapol_data = (u8 *)((u8 *)pEthHdr+sizeof(struct ethhdr_st));          
        size_t eapol_len= be_to_host16(*((u16 *)(eapol_data+2)));
