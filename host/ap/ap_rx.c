...
u8* ssv6xxx_host_rx_data_get_qos_ptr(CFG_HOST_RXPKT *rxpkt)
{
    return (u8 *)ssv_hal_get_rxpkt_qos_ptr(rxpkt);
}
...
extern u16 HCmdEng_GetRawRxDataOffset(CFG_HOST_RXPKT *p);
inline u32 ap_mode_fine_tune_rcpi(struct ap_rx_desp *rx_desp)
{
...
//Step1. Fill SA info in rx despriptor
        ...
        if (!rx_desp.sta || !test_sta_flag(rx_desp.sta, WLAN_STA_ASSOC)) {
            i802_sta_deauth((u8*)gDeviceInfo->APInfo->own_addr, (u8*)&pEthHdr->h_source, WLAN_REASON_CLASS2_FRAME_FROM_NONAUTH_STA);
        }
        ...
        pStaInfo = NULL;
        {
            ...
            (pStaInfo = APStaInfo_FindStaByAddr((ETHER_ADDR*)pEthHdr->h_dest)) )//ieee80211_deliver_skb
            {
                ...
                if(pRelayFrame)
                {
                    ...
                    os_frame_pull(pRelayFrame,(u32)CmdEng_GetRawRxDataOffset((CFG_HOST_RXPKT *)OS_FRAME_GET_DATA(pRelayFrame)));
