...
void APTxDespInit()
{
	/* AP_TOTAL_MAX_TX_DSP = g_host_cfg.pool_size = POOL_SIZE(72) */
	gDeviceInfo->APInfo->pApTxData = 
		(struct ap_tx_desp *)MALLOC(sizeof(struct ap_tx_desp) * AP_TOTAL_MAX_TX_DSP);
	ASSERT(gDeviceInfo->APInfo->pApTxData);
	MEMSET(gDeviceInfo->APInfo->pApTxData, 
			0, sizeof(struct ap_tx_desp) * AP_TOTAL_MAX_TX_DSP);
}
...
extern bool sta_info_recalc_tim(APStaInfo_st *sta);
...
void ssv6xxx_data_prepare_tx_data(struct ap_tx_desp *tx_desp, u32 *frame, CFG_HOST_TXREQ *host_txreq0, bool bAPFrame, u32 TxFlags)
{
	//int nHeaderLen;
//	u8 *dat = (u8*)host_txreq0;	
    ...
    rsvd0=ssv_hal_get_txreq0_rsvd0(host_txreq0);
    if(rsvd0&AP_PS_FRAME)
    {
        tx_desp->flags |= AP_TX_POLL_RESPONSE;        
        ssv_hal_set_txreq0_rsvd0(host_txreq0,0);
    }
    ...
    tx_desp->nDataLen = (u16)ssv_hal_get_txreq0_len(host_txreq0)-(tx_desp->data-(u8*)host_txreq0);
	

    if(1==ssv_hal_get_txreq0_f80211(host_txreq0))
    {
        //80211 frame
