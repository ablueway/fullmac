...
{
	MEMSET(ap, 0, sizeof(struct neighbor_ap_info));
...
static struct neighbor_ap_info * ap_ap_add(ApInfo_st *pApInfo, const ETHER_ADDR *addr)
{
	...
	MEMCPY((void*)ap->addr, (void*)addr, ETHER_ADDR_LEN);
...
struct ieee802_11_elems *elems)
{
	...
	MEMSET(ap->supported_rates, 0, AP_SUPP_RATES_MAX);
	len = 0;
	if (elems->supp_rates) {
	...
	if (elems->ext_supp_rates) {
		...
		MEMCPY(ap->supported_rates + len, elems->ext_supp_rates,
			  (u32)len2);
	}
	...
	ap->last_beacon = os_sys_jiffies(); //now.sec;
...
// 
 static void ap_list_timer(void *data1, void *data2)
 {
	...
	os_time_t now = os_sys_jiffies();
	int set_beacon = 0,i;

	int olbc = 0;			//Overlapping Legacy BSS Condition 
	int olbc_ht = 0;
    //if(gDeviceInfo->hw_mode != SSV6XXX_HWM_AP)
	if ((SSV6XXX_HWM_AP != gDeviceInfo->vif[0].hw_mode)&&(SSV6XXX_HWM_AP != gDeviceInfo->vif[1].hw_mode))
		return; 
 ...
 void reset_neighbor_apinfo(ApInfo_st *pApInfo)
 {
	 ...
	 MEMSET(pApInfo->neighbor_ap_list, 0, sizeof(struct neighbor_ap_info)*AP_NEIGHBOR_AP_TABLE_MAX_SIZE);
 }
...
{
	os_cancel_timer(ap_list_timer, (u32)pApInfo, 0);
	reset_neighbor_apinfo(pApInfo);
}
...
extern void ieee802_11_print_ssid(char *buf, const u8 *ssid, u8 len);
