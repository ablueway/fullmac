...
#ifdef __AP_DEBUG__
	void APStaInfo_PrintStaInfo(u32 idx);
#endif//__AP_DEBUG__
...
bool sta_info_recalc_tim(APStaInfo_st *sta)
{
...
void APStaInfo_Init()
{
	...
	gDeviceInfo->APInfo->StaConInfo = (struct APStaInfo *)MALLOC((u32)tsize);
    OS_MemSET((void *)gDeviceInfo->APInfo->StaConInfo,0,tsize);
	ASSERT(gDeviceInfo->APInfo->StaConInfo != NULL);
	...
	/* gMaxAID(8) */
	for (aid_idx = 0; aid_idx < gMaxAID; aid_idx++)
    {
		APStaInfo_StaReset(&gDeviceInfo->APInfo->StaConInfo[aid_idx]);
        gDeviceInfo->APInfo->StaConInfo[aid_idx].wsid_idx = gMaxAID;
/* in */        
#ifndef CONFIG_NO_WPA2         
        gDeviceInfo->APInfo->StaConInfo[aid_idx].addStaFlag = 0;
        gDeviceInfo->APInfo->StaConInfo[aid_idx].assocRsp = 0;
        gDeviceInfo->APInfo->StaConInfo[aid_idx].reassoc = 0;
#endif        
        OS_MutexInit(&gDeviceInfo->APInfo->StaConInfo[aid_idx].ps_tx_buf_mutex);
        OS_MutexInit(&gDeviceInfo->APInfo->StaConInfo[aid_idx].apsta_mutex);
    }
}
...
void change_sta_disassoc_to_deauth(ApInfo_st *pApInfo,APStaInfo_st *sta,bool timer_creat)
{
	...
	if (sta->nonerp_set) {
		...
		if (pApInfo->num_sta_non_erp == 0)
        {
            if(!pApInfo->olbc)
            {
                ...
                bool erp_protect = FALSE;
                _ssv6xxx_wifi_ioctl_Ext(SSV6XXX_HOST_CMD_SET_ERP_PROTECT, &erp_protect, sizeof(bool),TRUE,FALSE);      
            }
			set_beacon++;
	    }
	}
...
//Todo: HT
    ...
    {   
        u16 operation_mode = host_to_le16(pApInfo->ht_op_mode);
        _ssv6xxx_wifi_ioctl_Ext(SSV6XXX_HOST_CMD_SET_HT_PROTECT, &operation_mode, sizeof(bool),TRUE,FALSE);
		set_beacon++;
    }
...
APStaInfo_st *APStaInfo_add(ApInfo_st *pApInfo, const u8 *addr)
{
	...
	if (pApInfo->num_sta >= gMaxAID) {
        u32 idx=0,del_idx=gMaxAID;
        u8 max_arp_retry_count=0;
        ...
        if(del_idx<gMaxAID)
        {
            ...
            os_cancel_timer(ap_handle_timer,  (u32)pApInfo, (u32)StaInfo);
            //Send De-authentication frame                                                                                                  //----------------------------------->implement
...
{
    u32 rsvd0=ssv_hal_get_txreq0_rsvd0(tx_desp->host_txreq0);
    rsvd0|=AP_PS_FRAME;
    ssv_hal_set_txreq0_rsvd0(tx_desp->host_txreq0,rsvd0);
		...
		if (!driver_release_tids) {
			...
			while ((tx_desp = (struct ap_tx_desp *)list_q_deq_safe(&frames,&sta->ps_tx_buf_mutex))) {
				...
				if (more_data || !ap_tx_desp_queue_empty(&frames,&sta->ps_tx_buf_mutex))
					ssv_hal_set_txreq0_more_data(tx_desp->host_txreq0,1);
				else
					ssv_hal_set_txreq0_more_data(tx_desp->host_txreq0,0);


				//it's 80211 packet, need to add bit by hand
				if(ssv_hal_get_txreq0_f80211(tx_desp->host_txreq0))
				{
					struct ieee80211_hdr *hdr = (void*) tx_desp->data;

					if (ssv_hal_get_txreq0_more_data(tx_desp->host_txreq0))
						hdr->frame_control |=
				     ...
				     ap_tx_desp_queue_empty(&frames,&sta->ps_tx_buf_mutex))//Only set in Data frame ??? non data frame is last in queue
				{
					...
					if(1==ssv_hal_get_txreq0_qos(tx_desp->host_txreq0)){

						//802.11 frame
						if(1==ssv_hal_get_txreq0_f80211(tx_desp->host_txreq0))
...
extern H_APIs s32 ssv6xxx_get_rssi_by_mac(u8 *macaddr, u8 vif_idx);
...
void reset_ap_info()
{
    ...
    //reset
    MEMSET(gDeviceInfo->APInfo, 0, sizeof(struct ApInfo));    
