 ...
 * Response frames.
 */
extern const struct ap_wmm_ac_params ac_params;
u8* hostapd_eid_wmm(ApInfo_st *pApInfo, u8 *eid)
{
	u8 *pos = eid;
...
//		return eid;
	...
	wmm->qos_info = pApInfo->parameter_set_count & 0xf;
	...
	MEMCPY((void*)wmm->ac, (void*)&ac_params, sizeof(wmm->ac));
 ...
 */
int hostapd_eid_wmm_valid(ApInfo_st *pApInfo, const u8 *eid, size_t len)
{
	...
	if (len < sizeof(struct wmm_information_element)) {
		LOG_DEBUG("Too short WMM IE (len=%lu)\r\n",
			   (unsigned long) len);
		return 0;
	}
    ...
    #ifdef __AP_DEBUG__
		LOG_DEBUG("Validating WMM IE: OUI %02x:%02x:%02x  "
	...
	if (wmm->oui_subtype != WMM_OUI_SUBTYPE_INFORMATION_ELEMENT ||
	    wmm->version != WMM_VERSION) {
			LOG_DEBUG("Unsupported WMM IE Subtype/Version\r\n,%s",__func__);
		return 0;
	}
...
// static void wmm_send_action(ApInfo_st *pApInfo, const u8 *addr,
...
// 
// 
// static void wmm_addts_req(ApInfo_st *pApInfo,
...
// 
// 
// void hostapd_wmm_action(ApInfo_st *pApInfo,
