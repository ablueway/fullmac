...
extern struct APStaInfo *APStaInfo_add(struct ApInfo *pApInfo, const u8 *addr);
...
extern s32 if_sim_recv(void *dat, s32 len);
...
void TestCase_FillSTAReg(void)
{
	...
	for (i=0;i<3;i++)
	{
		...
		memcpy(&GHW_REGS.TBL_WSID[i].peer_mac, addr, 6);
...
void TestCase_AddAPSta(void)
{
	...
	for (i=0;i<3;i++)
	{
		...
		addr[4]=i;
		addr[5]=0x00;
		sta = APStaInfo_add(gAPInfo, (const u8*)&addr);
