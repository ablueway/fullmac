...
#define configUSE_TICK_HOOK				0 // Felix
#define configTICK_RATE_HZ				( OS_TICK_MAGIC_NUMBER ) /* In this non-real time simulated environment the tick frequency has to be at least a multiple of the Win32 tick frequency, and therefore very slow. */
...
#define configASSERT( x ) if( /*lint -save -e731 */ ( x ) == 0 /*lint -restore */) vAssertCalled(__FUNCTION__, __LINE__)
