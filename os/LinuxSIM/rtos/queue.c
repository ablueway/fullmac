	...
	for( ;; )
	{
		taskENTER_CRITICAL();
		{
			...
			if( pxQueue->uxMessagesWaiting < pxQueue->uxLength )
			{
				...
				queue then unblock it now. */ /*lint -save -e731 */
			...
			taskENTER_CRITICAL();
			{
				if( pxQueue->uxMessagesWaiting > ( unsigned portBASE_TYPE ) 0 )
				{
					...
					else
					{
						...
						any other tasks waiting for the data. */  /*lint -save -e731 */
						if( listLIST_IS_EMPTY( &( pxQueue->xTasksWaitingToReceive ) ) == pdFALSE )
						{ /*lint -restore */
...
static void prvUnlockQueue( xQueueHandle pxQueue )
{
	...
	taskENTER_CRITICAL();
	{
		...
		while( pxQueue->xTxLock > queueLOCKED_UNMODIFIED )
		{
			...
			blocked waiting for data to become available? */  /*lint -save -e731 */
			if( listLIST_IS_EMPTY( &( pxQueue->xTasksWaitingToReceive ) ) == pdFALSE )
			{ /*lint -restore */
				/* Tasks that are removed from the event list will get added to
				the pending ready list as the scheduler is still suspended. */
...
signed portBASE_TYPE xReturn;
	...
	portDISABLE_INTERRUPTS();
	{
		if( pxQueue->uxMessagesWaiting < pxQueue->uxLength )
		{
			...
			/* Were any co-routines waiting for data to become available? */  /*lint -save -e731 */
			if( listLIST_IS_EMPTY( &( pxQueue->xTasksWaitingToReceive ) ) == pdFALSE )
			{ /*lint -restore */
...
signed portBASE_TYPE xReturn;
	...
	portDISABLE_INTERRUPTS();
	{
		if( pxQueue->uxMessagesWaiting > ( unsigned portBASE_TYPE ) 0 )
		{
			...
			/* Were any co-routines waiting for space to become available? */ /*lint -save -e731 */
			if( listLIST_IS_EMPTY( &( pxQueue->xTasksWaitingToSend ) ) == pdFALSE )
			{ /*lint -restore */
				/* In this instance the co-routine could be placed directly
				into the ready list as we are within a critical section.
