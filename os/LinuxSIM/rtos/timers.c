...
portBASE_TYPE xReturn = pdFAIL;
	...
	if( xTimerQueue != NULL )
	{
		#if ( INCLUDE_xTimerGetTimerDaemonTaskHandle == 1 )
		{
			...
			xReturn = xTaskCreate( prvTimerTask, ( const signed char * ) "Tmr Svc Task", ( unsigned short ) configTIMER_TASK_STACK_DEPTH, NULL, ( unsigned portBASE_TYPE ) configTIMER_TASK_PRIORITY, &xTimerTaskHandle );	
		}
		#else
		{
			/* Create the timer task without storing its handle. */
			xReturn = xTaskCreate( prvTimerTask, ( const signed char * ) "Tmr Svc Task", ( unsigned short ) configTIMER_TASK_STACK_DEPTH, NULL, ( unsigned portBASE_TYPE ) configTIMER_TASK_PRIORITY, NULL);
		}
		#endif
	}
 ...
 *  timer. 
 */
portBASE_TYPE xTimerChangeSetting( xTimerHandle xTimer, portTickType ticks, unsigned portBASE_TYPE autoReload, void *args )
{
    xTIMER *pxTimer;
    
    if ( pdFALSE == xTimerIsTimerActive( xTimer ) )
    {
        pxTimer = (xTIMER *)xTimer;
...
/* Added by Felix: */
void xTimerGetSetting( xTimerHandle xTimer, u8 *autoReload, void **args )
{
    xTIMER *pxTimer = ( xTIMER * ) xTimer;
    if (autoReload != NULL)
        *autoReload = (u8)pxTimer->uxAutoReload;
    if (args != NULL)
        *args = pxTimer->pvTimerID;
}




portBASE_TYPE xTimerAutoReload( xTimerHandle xTimer, unsigned portBASE_TYPE autoReload )
{
    xTIMER *pxTimer;
    if ( pdFALSE == xTimerIsTimerActive( xTimer ) )
    {
