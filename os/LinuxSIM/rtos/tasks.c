...
/*-----------------------------------------------------------*/
	...
	const tskTCB * const pxTCB = ( tskTCB * ) xTask;
		...
		suspended list? */ /*lint -save -e731 */
		if( listIS_CONTAINED_WITHIN( &xSuspendedTaskList, &( pxTCB->xGenericListItem ) ) != pdFALSE )
		{ /*lint -restore */
			/* Has the task already been resumed from within an ISR? */ /*lint -save -e731 */
			if( listIS_CONTAINED_WITHIN( &xPendingReadyList, &( pxTCB->xEventListItem ) ) != pdTRUE )
			{ /*lint -restore */
				/* Is it in the suspended list because it is in the
				Suspended state?  It is possible to be in the suspended
				list because it is blocked on a task with no timeout
				specified. */ /*lint -save -e731 */
...
/*-----------------------------------------------------------*/
	...
	tskTCB *pxTCB;
		...
		currently executing task. */ /*lint -save -e774 */
...
portBASE_TYPE xReturn;
	...
	#if ( INCLUDE_xTaskGetIdleTaskHandle == 1 )
	{
		...
		xReturn = xTaskCreate( prvIdleTask, ( signed char * ) "IDLE_TASK", tskIDLE_STACK_SIZE, ( void * ) NULL, ( tskIDLE_PRIORITY | portPRIVILEGE_BIT ), &xIdleTaskHandle );
	}
	#else
	{
		/* Create the idle task without storing its handle. */
		xReturn = xTaskCreate( prvIdleTask, ( signed char * ) "IDLE_TASK", tskIDLE_STACK_SIZE, ( void * ) NULL, ( tskIDLE_PRIORITY | portPRIVILEGE_BIT ), NULL );
	}
	#endif
	...
	taskENTER_CRITICAL();
	{
		...
		if( uxSchedulerSuspended == ( unsigned portBASE_TYPE ) pdFALSE )
		{
			if( uxCurrentNumberOfTasks > ( unsigned portBASE_TYPE ) 0U )
			{
				...
				appropriate ready list. */ /*lint -save -e731 */
...
/*-----------------------------------------------------------*/
	...
	{
	volatile tskTCB *pxNextTCB;
    volatile const tskTCB *pxFirstTCB;
				...
				else
				{
					...
					if( ulStatsAsPercentage > 0UL )
					{
						...
						#else
						{
							...
							LOG_PRINTF() library can be used. */
...
/*-----------------------------------------------------------*/
	...
	{
	const tskTCB *pxTCB;
	const unsigned char *pcEndOfStack;
	unsigned portBASE_TYPE uxReturn;
...
/*-----------------------------------------------------------*/
    ...
    /*lint -restore */
		...
		if( pxTCB->uxPriority < pxCurrentTCB->uxPriority )
		{
			...
			be moved in to a new list. */ /*lint -save -e731 */
