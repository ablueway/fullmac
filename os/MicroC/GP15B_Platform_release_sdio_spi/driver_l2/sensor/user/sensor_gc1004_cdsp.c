...
void gc1004_cdsp_init(void)
{
	...
	drv_l2_sensor_set_mclkout(gc1004_cdsp_ops.info[0].mclk);
...
static void gc1004_cdsp_stream_on(INT32U index, INT32U bufA, INT32U bufB)
{
	...
	case 1:
	case 2:
		gc1004_sccb_write_table((regval8_t *)GC1004_Para_720P_30);
		break;
	...
	// mclk output
	drv_l2_sensor_set_mclkout(gc1004_cdsp_ops.info[index].mclk);
			
	// set cdsp format
	format.image_source = C_CDSP_FRONT;
	format.input_format = gc1004_cdsp_ops.info[index].input_format;
	format.output_format = gc1004_cdsp_ops.info[index].output_format;
	target_w = gc1004_cdsp_ops.info[index].target_w;
	target_h = gc1004_cdsp_ops.info[index].target_h;
	format.hpixel = gc1004_cdsp_ops.info[index].sensor_w;
	format.vline = gc1004_cdsp_ops.info[index].sensor_h;
	format.hoffset = gc1004_cdsp_ops.info[index].hoffset;
	format.voffset = gc1004_cdsp_ops.info[index].voffset;
	format.sensor_timing_mode = gc1004_cdsp_ops.info[index].interface_mode;
	format.sensor_hsync_mode = gc1004_cdsp_ops.info[index].hsync_active;
	format.sensor_vsync_mode = gc1004_cdsp_ops.info[index].vsync_active;
...
static drv_l2_sensor_info_t* gc1004_cdsp_get_info(INT32U index)
{
	if(index > (MAX_INFO_NUM - 1)) {
		...
		return (drv_l2_sensor_info_t*)&gc1004_cdsp_ops.info[index];
...
*********************************************/
const drv_l2_sensor_ops_t gc1004_cdsp_ops =
{
	SENSOR_GC1004_CDSP_NAME,		/* sensor name */
	gc1004_cdsp_init,
	gc1004_cdsp_uninit,
