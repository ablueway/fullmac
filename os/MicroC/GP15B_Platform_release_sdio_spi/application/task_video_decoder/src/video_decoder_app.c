...
static INT32S mjpeg_memory_alloc(mjpeg_info *pMjpeg)
{
	...
	g_jpeg_y_addr = (INT32U) gp_iram_malloc_align(y_size + cb_cr_size + cb_cr_size, 16);	
	if (g_jpeg_y_addr == 0) {
		g_jpeg_y_addr = (INT32U) gp_malloc_align(y_size + cb_cr_size + cb_cr_size, 16);
		if (g_jpeg_y_addr == 0) {
			return STATUS_FAIL;
		}
	}
		...
		if(jpeg_status & C_JPG_STATUS_DECODE_DONE) {	
			...
			while(1) {
				if(scaler_status == C_SCALER_STATUS_STOP) {
					scaler_status = drv_l2_scaler_trigger(SCALER_0, ENABLE, &scale, pMjpeg->boundary_color);
...
static INT32S move_data_wait_done(void)
{
	...
	status = (INT32S)OSQPend(dma_q, 0, &err);
	if(err != OS_NO_ERR) {
		while(1);
	}
	
	if(status != C_DMA_STATUS_DONE) {
		while(1);
	}
