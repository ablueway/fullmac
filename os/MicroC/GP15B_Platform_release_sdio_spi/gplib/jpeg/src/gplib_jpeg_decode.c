...
/*
* Function Name :  jpeg_decode_init
*
* Syntax : void jpeg_decode_init(void);
*
* Purpose :  to Initiate Jpeg decode module
*
* Parameters : <IN>  None
*                  
*              <OUT> None
...
/*
* Function Name :  jpeg_decode_parse_header
*
* Syntax : INT32S jpeg_decode_parse_header(INT8U *buf_start, INT32U len);
*
* Purpose :  to get the JPEG head information
*
* Parameters : <IN>  buf_start: jpeg buffer start pointer
*                    len: header length
*              <OUT> None
*
* Return : 0: JPEG_PARSE_OK
*          1: JPEG_PARSE_NOT_DONE
*          2: JPEG_PARSE_DONE
*          3: JPEG_PARSE_THUMBNAIL_DONE
*
* Note : buf_start points to buffer address of JPEG file
*
*/
...
INT32S huffman_gen_table(INT8U luminance, INT8U dc, INT8U selector)
{
	...
	if (dc) {								// DC
		if (luminance) {
			if (drv_l1_jpeg_huffman_dc_lum_mincode_set(0, 16, &huffman_mincode[0])) {
				DBG_PRINT("Calling to drv_l1_jpeg_huffman_dc_lum_mincode_set() failed\r\n");
				return JPEG_SET_HW_ERROR;
			}
			if (drv_l1_jpeg_huffman_dc_lum_valptr_set(0, 16, &huffman_valptr[0])) {
				DBG_PRINT("Calling to drv_l1_jpeg_huffman_dc_lum_valptr_set() failed\r\n");
				return JPEG_SET_HW_ERROR;
			}
		} else {
			if (drv_l1_jpeg_huffman_dc_chrom_mincode_set(0, 16, &huffman_mincode[0])) {
				DBG_PRINT("Calling to drv_l1_jpeg_huffman_dc_chrom_mincode_set() failed\r\n");
				return JPEG_SET_HW_ERROR;
			}
			if (drv_l1_jpeg_huffman_dc_chrom_valptr_set(0, 16, &huffman_valptr[0])) {
				DBG_PRINT("Calling to drv_l1_jpeg_huffman_dc_chrom_valptr_set() failed\r\n");
				return JPEG_SET_HW_ERROR;
			}
		}
	} else {								// AC
		if (luminance) {
			if (drv_l1_jpeg_huffman_ac_lum_mincode_set(0, 16, &huffman_mincode[0])) {
				DBG_PRINT("Calling to drv_l1_jpeg_huffman_ac_lum_mincode_set() failed\r\n");
				return JPEG_SET_HW_ERROR;
			}
			if (drv_l1_jpeg_huffman_ac_lum_valptr_set(0, 16, &huffman_valptr[0])) {
				DBG_PRINT("Calling to drv_l1_jpeg_huffman_ac_lum_valptr_set() failed\r\n");
				return JPEG_SET_HW_ERROR;
			}
		} else {
			if (drv_l1_jpeg_huffman_ac_chrom_mincode_set(0, 16, &huffman_mincode[0])) {
				DBG_PRINT("Calling to drv_l1_jpeg_huffman_ac_chrom_mincode_set() failed\r\n");
				return JPEG_SET_HW_ERROR;
			}
			if (drv_l1_jpeg_huffman_ac_chrom_valptr_set(0, 16, &huffman_valptr[0])) {
				DBG_PRINT("Calling to drv_l1_jpeg_huffman_ac_chrom_valptr_set() failed\r\n");
				return JPEG_SET_HW_ERROR;
			}
		}
	}
...
INT32S parse_restart_interval(void)
{
	...
	if (!parsing_thumbnail || decode_thumbnail) {
		if (drv_l1_jpeg_restart_interval_set(restart_interval)) {
			DBG_PRINT("Calling to drv_l1_jpeg_restart_interval_set() failed\r\n");
			return JPEG_SET_HW_ERROR;
		}
	}
...
INT32S parse_scan(void)
{
	...
	if (!parsing_thumbnail) {
		if (drv_l1_jpeg_image_size_set(image_width, image_height)) {
			DBG_PRINT("Calling to drv_l1_jpeg_image_size_set(%d, %d) failed\r\n", image_width, image_height);
			return JPEG_SET_HW_ERROR;
		}
	} else {
		if (drv_l1_jpeg_image_size_set(thumbnail_image_width, thumbnail_image_height)) {
			DBG_PRINT("Calling to drv_l1_jpeg_image_size_set(%d, %d) failed\r\n", thumbnail_image_width, thumbnail_image_height);
			return JPEG_SET_HW_ERROR;
		}
	}

	if (drv_l1_jpeg_yuv_sampling_mode_set((INT32U) mode)) {
		DBG_PRINT("Calling to drv_l1_jpeg_yuv_sampling_mode_set(0x%x) failed\r\n", mode);
		return JPEG_SET_HW_ERROR;
	}
	...
	if (scan_comp_number >= 1) {			// Luminance Y
		...
		if (quantization_installed[scan_comp_qtbl_index[0]]) {
			if (drv_l1_jpeg_quant_luminance_set(0, 64, &quantization_table[scan_comp_qtbl_index[0]][0])) {
				DBG_PRINT("Calling to drv_l1_jpeg_quant_luminance_set() failed\r\n");
				return JPEG_SET_HW_ERROR;
			}
		}
		...
		if (huffman_installed[(scan_comp_huffman_index[0]>>4)&0x1]) {		// 0 and 1 are DC, 2 and 3 are AC
			...
			if (drv_l1_jpeg_huffman_dc_lum_huffval_set(0, 16, &huffman_dc_values[(scan_comp_huffman_index[0]>>4)&0x1][0])) {
				DBG_PRINT("Calling to drv_l1_jpeg_huffman_dc_lum_huffval_set() failed\r\n");
				return JPEG_SET_HW_ERROR;
			}
		}
		...
		if (huffman_installed[0x2 | (scan_comp_huffman_index[0]&0x1)]) {		// 0 and 1 are DC, 2 and 3 are AC
			...
			if (drv_l1_jpeg_huffman_ac_lum_huffval_set(0, 256, &huffman_ac_values[scan_comp_huffman_index[0]&0x1][0])) {
				DBG_PRINT("Calling to drv_l1_jpeg_huffman_ac_lum_huffval_set() failed\r\n");
				return JPEG_SET_HW_ERROR;
			}
		}
	}
	...
	if (scan_comp_number >= 2) {
		...
		if (quantization_installed[scan_comp_qtbl_index[1]]) {
			if (drv_l1_jpeg_quant_chrominance_set(0, 64, &quantization_table[scan_comp_qtbl_index[1]][0])) {
				DBG_PRINT("Calling to drv_l1_jpeg_quant_chrominance_set() failed\r\n");
				return JPEG_SET_HW_ERROR;
			}
		} else if (quantization_installed[scan_comp_qtbl_index[2]]) {
			if (drv_l1_jpeg_quant_chrominance_set(0, 64, &quantization_table[scan_comp_qtbl_index[2]][0])) {
				DBG_PRINT("Calling to drv_l1_jpeg_quant_chrominance_set() failed\r\n");
				return JPEG_SET_HW_ERROR;
			}
		}
		...
		if (huffman_installed[(scan_comp_huffman_index[1]>>4)&0x1]) {		// 0 and 1 are DC, 2 and 3 are AC
			...
			if (drv_l1_jpeg_huffman_dc_chrom_huffval_set(0, 16, &huffman_dc_values[(scan_comp_huffman_index[1]>>4)&0x1][0])) {
				DBG_PRINT("Calling to drv_l1_jpeg_huffman_dc_chrom_huffval_set() failed\r\n");
				return JPEG_SET_HW_ERROR;
			}
		}
		...
		if (huffman_installed[0x2 | (scan_comp_huffman_index[1]&0x1)]) {		// 0 and 1 are DC, 2 and 3 are AC
			...
			if (drv_11_jpeg_huffman_ac_chrom_huffval_set(0, 256, &huffman_ac_values[scan_comp_huffman_index[1]&0x1][0])) {
				DBG_PRINT("Calling to drv_11_jpeg_huffman_ac_chrom_huffval_set() failed\r\n");
				return JPEG_SET_HW_ERROR;
			}
		}
	}
...
/*
* Function Name :  jpeg_decode_image_width_get
*
* Syntax : INT16U jpeg_decode_image_width_get(void);
*
* Purpose :  JPEG to get decode image width.
*
* Parameters : <IN>  None
*              <OUT> None
*
...
/*
* Function Name :  jpeg_decode_image_height_get
*
* Syntax : INT16U jpeg_decode_image_height_get(void);
*
* Purpose :  JPEG to get decode image height.
*
* Parameters : <IN>  None
*              <OUT> None
*
...
/*
* Function Name :  jpeg_decode_image_extended_width_get
*
* Syntax : INT16U jpeg_decode_image_extended_width_get(void);
*
* Purpose :  JPEG to get decode image width. according to YUV mode
*
* Parameters : <IN>  None
*              <OUT> None
*
* Return : Image width.
*
* Note : Get image extended width(according to YUV mode)
*
*/
...
/*
* Function Name :  jpeg_decode_image_extended_height_get
*
* Syntax : INT16U jpeg_decode_image_extended_height_get(void);
*
* Purpose :  JPEG to get decode image height. according to YUV mode
*
* Parameters : <IN>  None
*              <OUT> None
*
* Return : Image height.
*
* Note : Get image extended height(according to YUV mode)
*
*/
...
/*
* Function Name :  jpeg_decode_image_yuv_mode_get
*
* Syntax : INT16U jpeg_decode_image_yuv_mode_get(void);
*
* Purpose :  to get image YUV mode after header parsing finish
*
* Parameters : <IN>  None
*              <OUT> None
*
* Return : YUV mode.
*
* Note : Get image YUV mode after header parsing finish
*
*/
...
/*
* Function Name :  jpeg_decode_image_progressive_mode_get
*
* Syntax : INT8U jpeg_decode_image_progressive_mode_get(void);
*
* Purpose :  to get image  progressive mode
*
* Parameters : <IN>  None
*              <OUT> None
*
* Return : progressive mode: 0=baseline, 1=progressive
*
* Note : Get image image progressive mode
*
*/
...
/*
* Function Name :  jpeg_decode_image_orientation_get
*
* Syntax : INT8U jpeg_decode_image_orientation_get(void);
*
* Purpose :  to get image orientation
*
* Parameters : <IN>  None
*              <OUT> None
*
...
*
* Note : Get image orientation
*
*/
INT8U jpeg_decode_image_orientation_get(void)
{
	return orientation;
}

/*
* Function Name :  jpeg_decode_date_time_get
*
* Syntax : INT32S jpeg_decode_date_time_get(INT8U *target);
*
* Purpose :  to get image date time information
*
* Parameters : <IN>  None
*              <OUT> target: date time array
*
* Return :  0: Success  -1: Fail
*
* Note : Get image date time
*
*/
...
/*
* Function Name :  jpeg_decode_thumbnail_exist_get
*
* Syntax : INT8U jpeg_decode_thumbnail_exist_get(void);
*
* Purpose :  to get information about thumbnail image
*
* Parameters : <IN>  None
*              <OUT> None
*
* Return :  0:No thumbnail image, 1:Thumbnail image exists
*
* Note : 
*
*/
...
/*
* Function Name :  jpeg_decode_thumbnail_width_get
*
* Syntax : INT16U jpeg_decode_thumbnail_width_get(void);
*
* Purpose :  to get thumbnail image width
*
* Parameters : <IN>  None
*              <OUT> None
*
...
/*
* Function Name :  jpeg_decode_thumbnail_height_get
*
* Syntax : INT16U jpeg_decode_thumbnail_height_get(void);
*
* Purpose :  to get thumbnail image height
*
* Parameters : <IN>  None
*              <OUT> None
*
...
/*
* Function Name :  jpeg_decode_thumbnail_extended_width_get
*
* Syntax : INT16U jpeg_decode_thumbnail_extended_width_get(void);
*
* Purpose :  to get thumbnail extern width
*
* Parameters : <IN>  None
*              <OUT> None
*
* Return :  thumbnail extend width
*
* Note : 
*
*/
...
/*
* Function Name :  jpeg_decode_thumbnail_extended_height_get
*
* Syntax : INT16U jpeg_decode_thumbnail_extended_height_get(void);
*
* Purpose :  to get thumbnail extern height
*
* Parameters : <IN>  None
*              <OUT> None
*
* Return :  thumbnail extend height
*
* Note : 
*
*/
...
/*
* Function Name :  jpeg_decode_thumbnail_yuv_mode_get
*
* Syntax : INT16U jpeg_decode_thumbnail_yuv_mode_get(void);
*
* Purpose :  to get thumbnail yuv mode
*
* Parameters : <IN>  None
*              <OUT> None
*
* Return :  thumbnail yuv mode
*
* Note : 
*
*/
...
/*
* Function Name :  jpeg_decode_image_vlc_addr_get
*
* Syntax : INT8U * jpeg_decode_image_vlc_addr_get(void);
*
* Purpose :  to get image vlc start address 
*
* Parameters : <IN>  None
*              <OUT> None
*
* Return : jpeg image vlc address pointer
*
* Note : 
*
*/
...
/*
* Function Name :  jpeg_decode_output_set
*
* Syntax : INT32S jpeg_decode_output_set(INT32U y_addr, INT32U u_addr, INT32U v_addr, INT32U line);
*
* Purpose :  to set decode data output address and output data length
*
* Parameters : <IN>  y_addr: set Y value address
                     u_addr: set U value address
                     v_addr: set V value address
                     line:   set jpeg fifo line
*              <OUT> None
*
* Return : 0: Success, -1: False
*
* Note : Make sure parameters are at least 8-byte alignment
...
INT32S jpeg_decode_output_set(INT32U y_addr, INT32U u_addr, INT32U v_addr, INT32U line)
{
	...
	if(drv_l1_jpeg_fifo_line_set(line)) {
		return -1;
	}
	return 0;
}
...
/*
* Function Name :  jpeg_decode_thumbnail_image_enable
*
* Syntax : void jpeg_decode_thumbnail_image_enable(void);
*
* Purpose :  to enable decode thumbnail
*
* Parameters : <IN>  None
*              <OUT> None
*
...
/*
* Function Name :  jpeg_decode_thumbnail_image_disable
*
* Syntax : void jpeg_decode_thumbnail_image_disable(void);
*
* Purpose :  to disable decode thumbnail
*
* Parameters : <IN>  None
*              <OUT> None
*
...
/*
* Function Name :  jpeg_decode_clipping_range_set
*
* Syntax : INT32S jpeg_decode_clipping_range_set(INT16U start_x, INT16U start_y, INT16U width, INT16U height);
*
* Purpose :  to set image clipping range
*
* Parameters : <IN>  start_x: clipping image start offset x value
                     start_y: clipping image start offset y value
                     width:   clipping image width
                     height:  clipping image height
*              <OUT> None
*
* Return : 0: Success, -1: Over range or Not 8-byte alignment
*
* Note : Make sure parameters are 8-byte alignment
*
*/
INT32S jpeg_decode_clipping_range_set(INT16U start_x, INT16U start_y, INT16U width, INT16U height)
{
	return drv_l1_jpeg_image_clipping_range_set(start_x, start_y, width, height);
}

/*
* Function Name :  jpeg_decode_clipping_mode_enable
*
* Syntax : INT32S jpeg_decode_clipping_mode_enable(void);
*
* Purpose :  to enable jpeg decode clipping mode
*
* Parameters : <IN>  None
*              <OUT> None
*
* Return : 0: Success, -1: Working
*
* Note :  Not allowed to change setting when JPEG is working
*
*/
INT32S jpeg_decode_clipping_mode_enable(void)
{
	return drv_l1_jpeg_clipping_mode_enable();
}

/*
* Function Name :  jpeg_decode_clipping_mode_disable
*
* Syntax : INT32S jpeg_decode_clipping_mode_disable(void);
*
* Purpose :  to disable jpeg decode clipping mode
*
* Parameters : <IN>  None
*              <OUT> None
*
...
{
	return drv_l1_jpeg_clipping_mode_disable();
}

/*
* Function Name :  jpeg_decode_level2_scaledown_enable
*
* Syntax : INT32S jpeg_decode_level2_scaledown_enable(void);
*
* Purpose :  to enable scaledown
*
* Parameters : <IN>  None
*              <OUT> None
*
...
{
	return drv_l1_jpeg_level2_scaledown_mode_enable();
}

/*
* Function Name :  jpeg_decode_level2_scaledown_disable
*
* Syntax : INT32S jpeg_decode_level2_scaledown_disable(void);
*
* Purpose :  to disable scaledown
*
* Parameters : <IN>  None
*              <OUT> None
*
...
{
	return drv_l1_jpeg_level2_scaledown_mode_disable();
}
...
* Syntax : INT32S jpeg_decode_level2_scaledown_enable(void);
*
* Purpose :  to enable scaledown,for separate yuv scaler down
*
* Parameters : <IN>  None
*              <OUT> None
*
...
{
	//return drv_l1_jpeg_level8_scaledown_mode_enable();
	return -1;
}

/*
* Function Name :  jpeg_decode_level8_scaledown_disable
*
* Syntax : INT32S jpeg_decode_level8_scaledown_disable(void);
*
* Purpose :  to disable scaledown,for separate yuv scaler down
*
* Parameters : <IN>  None
*              <OUT> None
*
...
{
	//return drv_l1_jpeg_level8_scaledown_mode_disable();
	return -1;
}

/*
* Function Name :  jpeg_decode_vlc_maximum_length_set
*
* Syntax : INT32S jpeg_decode_vlc_maximum_length_set(INT32U len);
*
* Purpose :  to get image vlc start address 
*
* Parameters : <IN>  len: Max lenghth of vlc to set
*              <OUT> None
*
* Return : 0: Success, -1: False
*
* Note : if len > vlc total length max,len = vlc total len max and reuturn -1.
*        Not allowd to change setting when JPEG is working,and reutrn -1.
*
*/
INT32S jpeg_decode_vlc_maximum_length_set(INT32U len)
{
	return drv_l1_jpeg_vlc_maximum_length_set(len);
}

// Decodes complete JPEG VLC(variable length coding)
/*
* Function Name :  jpeg_decode_once_start
*
* Syntax : INT32S jpeg_decode_once_start(INT8U *buf, INT32U len);
*
* Purpose :  decompression start once for JPEG VLC (variable length coding)
*
* Parameters : <IN>  buf:  vlc address
                     len:  VLC maximum length
*              <OUT> None
*
* Return : 0: Success, -1: Working or length over range
*
* Note :  
*
*/
INT32S jpeg_decode_once_start(INT8U *buf, INT32U len)
{
	...
	if (drv_l1_jpeg_vlc_addr_set((INT32U) buf)) {
		DBG_PRINT("Calling to drv_l1_jpeg_vlc_addr_set() failed\r\n");
		return -1;
	}
	if (drv_l1_jpeg_vlc_maximum_length_set(len)) {
		DBG_PRINT("Calling to drv_l1_jpeg_vlc_maximum_length_set() failed\r\n");
		return -1;
	}
	if (drv_l1_jpeg_decompression_start(NULL)) {
		DBG_PRINT("Calling to drv_l1_jpeg_decompression_start() failed\r\n");
		return -1;
	}
...
/*
* Function Name :  jpeg_decode_on_the_fly_start
*
* Syntax : INT32S jpeg_decode_on_the_fly_start(INT8U *buf, INT32U len);
*
* Purpose :  decompression restart multi VLC (variable length coding)
*
* Parameters : <IN>  buf:  vlc buffer address
                     len:  VLC maximum length
*              <OUT> None
*
* Return : 0: Success, -1: Working or length over range
*
* Note :  Parameter buf must be 16-byte alignment
*
*/
INT32S jpeg_decode_on_the_fly_start(INT8U *buf, INT32U len)
{
	if (jpeg_decode_started) {
		if ((INT32U) buf & 0xF) {
			...
			return -1;
		} else if (drv_l1_jpeg_multi_vlc_input_restart((INT32U) buf, len)) {
			DBG_PRINT("Calling to drv_l1_jpeg_multi_vlc_input_restart() failed\r\n");
			return -1;
		}
		...
		if (drv_l1_jpeg_multi_vlc_input_init(len)) {
			DBG_PRINT("Calling to drv_l1_jpeg_multi_vlc_input_init() failed\r\n");
			return -1;
		}
		if (drv_l1_jpeg_decompression_start(NULL)) {
			DBG_PRINT("Calling to drv_l1_jpeg_decompression_start() failed\r\n");
...
// Decodes a sequence of JPEG VLC(variable length coding) buffers
INT32S jpeg_decode_on_the_fly_start2(INT8U *buf, INT32U len)
{
	if (jpeg_decode_started) {
		if ((INT32U) buf & 0xF) {
			DBG_PRINT("Parameter buf is not 16-byte alignment in jpeg_decode_on_the_fly_start()\r\n");
...
/*
* Function Name :  jpeg_decode_output_restart
*
* Syntax : INT32S jpeg_decode_output_restart(void);
*
* Purpose :  to start JPEG to output to next FIFO
*
* Parameters : <IN>  None
*              <OUT> None
*
* Return : 0: Success, -1: False
*
* Note :  If FIFO output mode is used, 
*         JPEG will stop when output FIFO is full. 
*         Clear the pending bit to restart the JPEG decompression engine.
*
*/
...
#if 1
	INT32S mask;
	INT32U x = R_JPG_CTRL & C_JPG_CTRL_DECOMPRESS ;
	if (x != C_JPG_CTRL_DECOMPRESS) {
		return -1;
	}
	if ((R_JPG_RING_FIFO & C_JPG_FIFO_ENABLE)!=C_JPG_FIFO_ENABLE) {
		return -1;
	}

	mask = drv_l1_jpeg_device_protect();
	R_JPG_INT_FLAG = C_JPG_FLAG_DECODE_OUTPUT_FULL;		// Clear pending bit, JPEG will output to next FIFO
  	R_JPG_INT_CTRL |= C_JPG_INT_DECODE_OUTPUT_FULL;
	drv_l1_jpeg_device_unprotect(mask);

  	return 0;
#else
	return drv_l1_jpeg_yuv_output_restart();
#endif
}

/*
* Function Name :  jpeg_decode_stop
*
* Syntax : void jpeg_decode_stop(void);
*
* Purpose :  to Stop JPEG engine
*
* Parameters : <IN>  None
*              <OUT> None
*
...
/*
* Function Name :  jpeg_decode_status_query
*
* Syntax : INT32S jpeg_decode_status_query(INT32U wait);
*
* Purpose :  to get the current status of JPEG engine
*
* Parameters : <IN>  wait: 0: accept msg,other: wait pand msg
*              <OUT> None
*
* Return : JPEG engine status
*
* Note : 
*
*/
