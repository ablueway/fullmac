...
rem "C:\Program Files\7-Zip\7z.exe" x %~dp0\os\MicroC\gp3260xxa_15B_release_v0.0.2.7z -o%~dp0\os\MicroC
rem "C:\Program Files\7-Zip\7z.exe" x %~dp0\os\MicroC\gp3260xxa_15B_release_v0.0.2\tools\make_tools.7z -o%~dp0\os\MicroC\gp3260xxa_15B_release_v0.0.2\tools
...
rem del /f/q %~dp0\host\version.c
rem del /f/q %~dp0\host\main.c
rem del /f/q %~dp0\host\ethermac.c
...
rem del /f/q %~dp0\host\lib\*.c
rem del /f/q %~dp0\host\lib\apps\*.c
rem del /f/q %~dp0\host\core\*.c
rem del /f/q %~dp0\host\drv\*.c
rem del /f/q %~dp0\host\ap\*.c
rem del /f/q %~dp0\host\ap\common\*.c
rem rd /s/q %~dp0\host\ap\crypto
rem del /f/q %~dp0\host\app\netapp\iperf3.0\src\*.c
rem del /f/q %~dp0\os\MicroC\gp3260xxa_15B_release_v0.0.2\project\GP3260xxa\gp3260xxa_platform_demo_release_captureRaw_lincorr.mcp
rem del /f/q %~dp0\host\app\netapp\udhcp\*.c
rem del /f/q %~dp0\host\app\netmgr\*.c
rem del /f/q %~dp0\host\app\netmgr\SmartConfig\*.c
rem del /f/q %~dp0\host\app\netmgr\SmartConfig\iComm\core\*.c

rem rd /s/q %~dp0\host\os_wrapper\LinuxSIM
rem rd /s/q %~dp0\host\os_wrapper\FreeRTOS
rem rd /s/q %~dp0\host\os_wrapper\AKOS

rem rd /s/q %~dp0\host\platform\LinuxSIM

rem rd /s/q %~dp0\host\drv\ftdi-spi
rem rd /s/q %~dp0\host\drv\sdio_linux
rem rd /s/q %~dp0\host\drv\sdio_linux_hci
rem rd /s/q %~dp0\host\drv\sdio_win32

rem rd /s/q %~dp0\host\ap
rem rd /s/q %~dp0\host\core
rem rd /s/q %~dp0\host\include\priv\hw

rem rd /s/q %~dp0\os\LinuxSIM
rem rd /s/q %~dp0\os\FreeRTOS

rem rd /s/q %~dp0\script

rem rd /s/q %~dp0\host\app\netapp\iperf3.0

rem rd /s/q %~dp0\doc
rem rd /s/q %~dp0\.svn

rem del /f/q %~dp0\.cproject
rem del /f/q %~dp0\.gdbinit
rem del /f/q %~dp0\.project
rem del /f/q %~dp0\build_gp.bat
rem del /f/q %~dp0\build_gp.sh
rem del /f/q %~dp0\build_gp19.bat
rem del /f/q %~dp0\Makefile
rem del /f/q %~dp0\Makefile.release
rem del /f/q %~dp0\readme.txt
rem del /f/q %~dp0\release.sh
rem del /f/q %~dp0\ssv6200-uart.bin

rem code warrior for axf

rem SET PATH=%PATH%;C:\Program Files\ARM\ADSv1_2\Bin
rem cmdide.exe  %~dp0\os\MicroC\gp3260xxa_15B_release_v0.0.2\project\GP3260xxa\gp3260xxa_platform_demo_release_wifi.mcp /r /b /q"
